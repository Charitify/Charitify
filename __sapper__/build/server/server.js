...
const css$4 = {
	code: ".modal.svelte-13jhqpw.svelte-13jhqpw{z-index:8;position:fixed;top:0;left:0;width:100%;height:100%;display:-webkit-box;display:-ms-flexbox;display:flex;overflow-x:hidden;overflow-y:auto;-webkit-box-align:center;-ms-flex-align:center;align-items:center;-webkit-box-pack:center;-ms-flex-pack:center;justify-content:center;-webkit-box-orient:vertical;-webkit-box-direction:normal;-ms-flex-direction:column;flex-direction:column;-ms-touch-action:manipulation;touch-action:manipulation;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;background-color:rgba(var(--color-black), .75);outline:50px solid rgba(var(--color-black), .75);-webkit-transition-timing-function:ease-out;transition-timing-function:ease-out;opacity:0;pointer-events:none}.modal.active.svelte-13jhqpw.svelte-13jhqpw,.modal-header.active.svelte-13jhqpw.svelte-13jhqpw{opacity:1;pointer-events:auto}.modal-inner.svelte-13jhqpw.svelte-13jhqpw{display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-orient:vertical;-webkit-box-direction:normal;-ms-flex-direction:column;flex-direction:column;-webkit-box-align:stretch;-ms-flex-align:stretch;align-items:stretch;-webkit-box-pack:stretch;-ms-flex-pack:stretch;justify-content:stretch;overflow:hidden;background-color:rgba(var(--theme-color-primary))}.small.svelte-13jhqpw .modal-inner.svelte-13jhqpw{width:200px;border-radius:var(--border-radius-big)}.medium.svelte-13jhqpw .modal-inner.svelte-13jhqpw{width:calc(100vw - var(--screen-padding) * 2);border-radius:var(--border-radius-big)}.big.svelte-13jhqpw .modal-inner.svelte-13jhqpw{width:calc(100% - var(--screen-padding) * 2);height:calc(100% - var(--screen-padding) * 2);border-radius:var(--border-radius-big)}.full.svelte-13jhqpw.svelte-13jhqpw{-webkit-box-align:stretch;-ms-flex-align:stretch;align-items:stretch;-webkit-box-pack:stretch;-ms-flex-pack:stretch;justify-content:stretch}.full.svelte-13jhqpw .modal-inner.svelte-13jhqpw{-webkit-box-flex:0;-ms-flex:none;flex:none;width:100%;min-height:100%;border-radius:0}.modal-header.svelte-13jhqpw.svelte-13jhqpw{-webkit-transform:translateZ(0);transform:translateZ(0);z-index:9;position:fixed;top:0;left:0;width:100%;display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-ms-flex-align:center;align-items:center;-webkit-box-pack:justify;-ms-flex-pack:justify;justify-content:space-between;color:rgb(var(--color-white));background-color:rgb(var(--color-info));opacity:0;pointer-events:none;-webkit-transform-origin:50% 50vh;transform-origin:50% 50vh}.modal-header.svelte-13jhqpw .close.svelte-13jhqpw{font-size:24px;display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-ms-flex-align:center;align-items:center;-webkit-box-pack:center;-ms-flex-pack:center;justify-content:center;width:60px;height:60px}",
	map: "{\"version\":3,\"file\":\"Modal.svelte\",\"sources\":[\"Modal.svelte\"],\"sourcesContent\":[\"<script>\\n    import { createEventDispatcher, tick } from 'svelte'\\n    import { fly } from \\\"svelte/transition\\\";\\n    import { Swipe } from '@services'\\n    import { safeGet, classnames, delay, bodyScroll, stopPropagationInRanges } from \\\"@utils\\\";\\n    import { modals } from \\\"@store\\\";\\n    import Portal from \\\"./Portal.svelte\\\";\\n    import Br from \\\"./Br.svelte\\\";\\n\\n    const dispatch = createEventDispatcher()\\n    \\n    const DURATION = 250\\n    const THRESHOLD = 100\\n    const THRESHOLD_RANGES = { x: [0, 100], y: [1, 99] }\\n    const START_POSITION = {\\n        x: 300,\\n        y: 0\\n    }\\n\\n    export let id\\n    export let ref = null\\n    export let size = 'full'    // small/medium/big/full\\n    export let swipe = []       // up down left right all\\n    export let open = null\\n    export let startPosition = START_POSITION\\n    export let blockBody = true\\n    export let withHeader = true\\n\\n    let active\\n    let refHeader\\n    let isBodyBlocked = false\\n    let isAllowed = {\\n        up: true,\\n        down: false,\\n        left: true,\\n        right: true,\\n    }\\n\\n    $: isSwipe = {\\n        up: safeGet(() => swipe.includes('up') || swipe.includes('all')),\\n        down: safeGet(() => swipe.includes('down') || swipe.includes('all')),\\n        left: safeGet(() => swipe.includes('left') || swipe.includes('all')),\\n        right: safeGet(() => swipe.includes('right') || swipe.includes('all')),\\n    }\\n    $: scrollY = ref && ref.scrollTop\\n    $: active = safeGet(() => open !== null ? open : $modals[`modal-${id}`].open, null)\\n    $: classProp = classnames('modal', size, { active })\\n    $: onActiveChange(active)\\n    $: blockScroll(ref)\\n\\n    function blockScroll(modal) {\\n        if (blockBody && active && !isBodyBlocked) {\\n            bodyScroll.disableScroll(modal, { extraLock: size === 'full' });\\n            isBodyBlocked = true\\n            modal && (modal.scrollTop = 0)\\n            isAllowed = {\\n                up: true,\\n                down: false,\\n                left: true,\\n                right: true,\\n            }\\n        } else if (blockBody && !active && isBodyBlocked) {\\n            bodyScroll.enableScroll(modal, { extraLock: size === 'full' });\\n            isBodyBlocked = false\\n        }\\n    }\\n\\n    async function onActiveChange(active) {\\n        if (active) {\\n            setDuration(ref, DURATION)\\n            setDuration(refHeader, DURATION)\\n            setTimeout(() => setDuration(ref, 0), DURATION)\\n            setTimeout(() => setDuration(refHeader, 0), DURATION)\\n            drawTransform(ref, 0, 0)\\n            drawTransform(refHeader, 0, 0)\\n            drawOpacity(ref, 0, 0)\\n            drawOpacity(refHeader, 0, 0)\\n            blockScroll(ref)\\n            await tick()\\n            dispatch('open')\\n        } else {\\n            blockScroll(ref)\\n            await tick()\\n            dispatch('close')\\n        }\\n    }\\n\\n    function setActive(isActive) {\\n        if (open !== null) open = isActive\\n        modals.update(s => ({ ...s, [`modal-${id}`]: { open: isActive } }))\\n    }\\n\\n    let xSwipe = 0\\n    let ySwipe = 0\\n\\n    function addSwipe(el) {\\n        stopPropagationInRanges(el, THRESHOLD_RANGES, ({ x, y }) => {\\n            isAllowed = {\\n                up: y <= THRESHOLD_RANGES.y[0],\\n                down: y >= THRESHOLD_RANGES.y[1],\\n                left: x <= THRESHOLD_RANGES.x[0] || x >= THRESHOLD_RANGES.x[1],\\n                right: x <= THRESHOLD_RANGES.x[0] || x >= THRESHOLD_RANGES.x[1],\\n            } \\n        })\\n\\n        new Swipe(el)\\n                .run()\\n                .onUp(isSwipe.up ? handleVerticalSwipe : null)\\n                .onDown(isSwipe.down ? handleVerticalSwipe : null)\\n                .onLeft(isSwipe.left ? handleHorizontalSwipe : null)\\n                .onRight(isSwipe.right ? handleHorizontalSwipe : null)\\n                .onTouchEnd(async () => {\\n                    if (xSwipe > THRESHOLD) {\\n                        setActive(false)\\n                        drawOpacity(el, xSwipe + 50, ySwipe)\\n                        drawOpacity(refHeader, xSwipe + 50, ySwipe)\\n                        drawTransform(el, xSwipe + 50, ySwipe)\\n                        drawTransform(refHeader, xSwipe + 50, ySwipe)\\n                        await delay(DURATION)\\n                    } else if (xSwipe < -THRESHOLD) {\\n                        setActive(false)\\n                        drawOpacity(el, xSwipe - 50, ySwipe)\\n                        drawOpacity(refHeader, xSwipe - 50, ySwipe)\\n                        drawTransform(el, xSwipe - 50, ySwipe)\\n                        drawTransform(refHeader, xSwipe - 50, ySwipe)\\n                        await delay(DURATION)\\n                    }\\n                    \\n                    if (ySwipe > THRESHOLD) {\\n                        setActive(false)\\n                        drawOpacity(el, xSwipe, ySwipe + 50)\\n                        drawOpacity(refHeader, xSwipe, ySwipe + 50)\\n                        drawTransform(el, xSwipe, ySwipe + 50)\\n                        drawTransform(refHeader, xSwipe, ySwipe + 50)\\n                        await delay(DURATION)\\n                    } else if (ySwipe < -THRESHOLD) {\\n                        setDuration(el, DURATION)\\n                        setDuration(refHeader, DURATION)\\n                        setTimeout(() => setDuration(el, 0), DURATION)\\n                        setTimeout(() => setDuration(refHeader, 0), DURATION)\\n                        setActive(false)\\n                        drawOpacity(el, xSwipe, ySwipe - 50)\\n                        drawOpacity(refHeader, xSwipe, ySwipe - 50)\\n                        drawTransform(el, xSwipe, ySwipe - 50)\\n                        drawTransform(refHeader, xSwipe, ySwipe - 50)\\n                        await delay(DURATION)\\n                    }\\n\\n                    if (xSwipe <= THRESHOLD && xSwipe >= -THRESHOLD && ySwipe <= THRESHOLD && ySwipe >= -THRESHOLD) {\\n                        setDuration(el, DURATION)\\n                        setDuration(refHeader, DURATION)\\n                        setTimeout(() => setDuration(el, 0), DURATION)\\n                        setTimeout(() => setDuration(refHeader, 0), DURATION)\\n                        drawTransform(el, 0, 0)\\n                        drawTransform(refHeader, 0, 0)\\n                    } else {\\n                        setStartPosition()\\n                    }\\n\\n                    xSwipe = 0\\n                    ySwipe = 0\\n                    el.style.opacity = null\\n                    refHeader.style.opacity = null\\n                })\\n    }\\n\\n    function handleVerticalSwipe(yDown, yUp, evt, el) {\\n        const dir = yUp - yDown\\n        if (!isAllowed.up && dir > 0 || !isAllowed.down && dir < 0) return\\n        ySwipe = dir\\n        drawTransform(el, xSwipe, ySwipe)\\n        drawTransform(refHeader, xSwipe, ySwipe)\\n        drawOpacity(el, xSwipe, ySwipe)\\n        drawOpacity(refHeader, xSwipe, ySwipe)\\n    }\\n    function handleHorizontalSwipe(xDown, xUp, evt, el) {\\n        const dir = xUp - xDown\\n        if (!isAllowed.left && dir > 0 || !isAllowed.right && dir < 0) return\\n        xSwipe = dir\\n        drawTransform(el, xSwipe, ySwipe)\\n        drawTransform(refHeader, xSwipe, ySwipe)\\n        drawOpacity(el, xSwipe, ySwipe)\\n        drawOpacity(refHeader, xSwipe, ySwipe)\\n    }\\n\\n    function setStartPosition() {\\n        drawTransform(ref, startPosition.x, startPosition.y)\\n        drawTransform(refHeader, startPosition.x, startPosition.y)\\n    }\\n\\n    function drawTransform(el, x, y) {\\n        const delta = Math.abs(x) > Math.abs(y) ? x : y\\n        let scale = 1 - Math.abs(delta / window.innerHeight)\\n        el && (el.style.transform = `matrix(${scale}, 0, 0, ${scale}, ${x}, ${y})`)\\n    }\\n     function setDuration(el, ms) {\\n        el && (el.style.transitionDuration = `${ms}ms`)\\n    }\\n    function drawOpacity(el, x, y) {\\n        const delta = Math.abs(x) > Math.abs(y) ? x : y\\n        el && (el.style.opacity = 1 - Math.min(Math.abs(delta / (THRESHOLD * 1.5)), 1))\\n    }\\n\\n    function appear(node, params) {\\n\\t\\tconst existingTransform = getComputedStyle(node).transform.replace('none', '');\\n        const getScale = t => .6 + .4 * t\\n        const getX = t => startPosition.x - startPosition.x * t\\n\\t\\treturn {\\n\\t\\t\\tduration: DURATION,\\n\\t\\t\\tcss: (t) => `opacity: ${t}; transform: matrix(${getScale(t)}, 0, 0, ${getScale(t)}, ${getX(t)}, 0)`\\n\\t\\t};\\n    }\\n\\n    function onCloseModal() {\\n        setDuration(ref, DURATION)\\n        setDuration(refHeader, DURATION)\\n        setTimeout(() => setDuration(ref, 0), DURATION)\\n        setTimeout(() => setDuration(refHeader, 0), DURATION)\\n        setStartPosition()\\n        drawOpacity(ref, startPosition.x, startPosition.y)\\n        drawOpacity(refHeader, startPosition.x, startPosition.y)\\n        setTimeout(() => setActive(false), DURATION)\\n    }\\n</script>\\n\\n{#if active !== null}\\n    <Portal>\\n        <div\\n            id={`modal-${id}`}\\n            bind:this={ref}\\n            aria-hidden=\\\"true\\\" \\n            class={classProp}\\n            use:addSwipe\\n            in:appear\\n            on:click={setActive.bind(null, false)}\\n        >\\n            {#if withHeader}\\n                <Portal>\\n                    <slot name=\\\"header\\\">\\n                        <button \\n                            type=\\\"button\\\"\\n                            class={classnames('modal-header', { active })}\\n                            in:appear\\n                            bind:this={refHeader}\\n                            on:click={onCloseModal}\\n                        >\\n                            <h2 style=\\\"padding: 15px 20px\\\">Закрити</h2>\\n                            <span class=\\\"close\\\">&#10005;</span>\\n                        </button>\\n                    </slot>\\n                </Portal>   \\n                <Br size=\\\"60\\\"/>\\n            {/if}\\n            <div\\n                class=\\\"modal-inner\\\"\\n                tabindex=\\\"-1\\\"\\n                role=\\\"dialog\\\"\\n                aria-modal=\\\"true\\\"\\n                aria-labelledby=\\\"модальне вікно\\\"\\n                on:click={e => e.stopPropagation()}\\n            >\\n                <slot props={safeGet(() => $modals[`modal-${id}`], {}, true)}/>\\n            </div>\\n        </div>\\n    </Portal>\\n{/if}\\n\\n<style>\\n    .modal {\\n        z-index: 8;\\n        position: fixed;\\n        top: 0;\\n        left: 0;\\n        width: 100%;\\n        height: 100%;\\n        display: -webkit-box;\\n        display: -ms-flexbox;\\n        display: flex;\\n        overflow-x: hidden;\\n        overflow-y: auto;\\n        -webkit-box-align: center;\\n            -ms-flex-align: center;\\n                align-items: center;\\n        -webkit-box-pack: center;\\n            -ms-flex-pack: center;\\n                justify-content: center;\\n        -webkit-box-orient: vertical;\\n        -webkit-box-direction: normal;\\n            -ms-flex-direction: column;\\n                flex-direction: column;\\n        -ms-touch-action: manipulation;\\n            touch-action: manipulation;\\n        -webkit-user-select: none;\\n           -moz-user-select: none;\\n            -ms-user-select: none;\\n                user-select: none;\\n        background-color: rgba(var(--color-black), .75);\\n        outline: 50px solid rgba(var(--color-black), .75);\\n        -webkit-transition-timing-function: ease-out;\\n                transition-timing-function: ease-out;\\n        opacity: 0;\\n        pointer-events: none;\\n    }\\n\\n    .modal.active, .modal-header.active {\\n        opacity: 1;\\n        pointer-events: auto;\\n    }\\n\\n    .modal-inner {\\n        display: -webkit-box;\\n        display: -ms-flexbox;\\n        display: flex;\\n        -webkit-box-orient: vertical;\\n        -webkit-box-direction: normal;\\n            -ms-flex-direction: column;\\n                flex-direction: column;\\n        -webkit-box-align: stretch;\\n            -ms-flex-align: stretch;\\n                align-items: stretch;\\n        -webkit-box-pack: stretch;\\n            -ms-flex-pack: stretch;\\n                justify-content: stretch;\\n        overflow: hidden;\\n        background-color: rgba(var(--theme-color-primary));\\n    }\\n    .small .modal-inner {\\n        width: 200px;\\n        border-radius: var(--border-radius-big);\\n    }\\n\\n    .medium .modal-inner {\\n        width: calc(100vw - var(--screen-padding) * 2);\\n        border-radius: var(--border-radius-big);\\n    }\\n    .big .modal-inner {\\n        width: calc(100% - var(--screen-padding) * 2);\\n        height: calc(100% - var(--screen-padding) * 2);\\n        border-radius: var(--border-radius-big);\\n    }\\n\\n    .full {\\n        -webkit-box-align: stretch;\\n            -ms-flex-align: stretch;\\n                align-items: stretch;\\n        -webkit-box-pack: stretch;\\n            -ms-flex-pack: stretch;\\n                justify-content: stretch;\\n    }\\n\\n    .full .modal-inner {\\n        -webkit-box-flex: 0;\\n            -ms-flex: none;\\n                flex: none;\\n        width: 100%;\\n        min-height: 100%;\\n        border-radius: 0;\\n    }\\n\\n    .modal-header {\\n        -webkit-transform: translateZ(0);\\n                transform: translateZ(0);\\n        z-index: 9;\\n        position: fixed;\\n        top: 0;\\n        left: 0;\\n        width: 100%;\\n        display: -webkit-box;\\n        display: -ms-flexbox;\\n        display: flex;\\n        -webkit-box-align: center;\\n            -ms-flex-align: center;\\n                align-items: center;\\n        -webkit-box-pack: justify;\\n            -ms-flex-pack: justify;\\n                justify-content: space-between;\\n        color: rgb(var(--color-white));\\n        background-color: rgb(var(--color-info));\\n        opacity: 0;\\n        pointer-events: none;\\n        -webkit-transform-origin: 50% 50vh;\\n                transform-origin: 50% 50vh;\\n    }\\n\\n    .modal-header .close {\\n        font-size: 24px;\\n        display: -webkit-box;\\n        display: -ms-flexbox;\\n        display: flex;\\n        -webkit-box-align: center;\\n            -ms-flex-align: center;\\n                align-items: center;\\n        -webkit-box-pack: center;\\n            -ms-flex-pack: center;\\n                justify-content: center;\\n        width: 60px;\\n        height: 60px;\\n    }\\n\\n/*# sourceMappingURL=data:application/json;base64, */</style>   \"],\"names\":[],\"mappings\":\"AA4QI,MAAM,8BAAC,CAAC,AACJ,OAAO,CAAE,CAAC,CACV,QAAQ,CAAE,KAAK,CACf,GAAG,CAAE,CAAC,CACN,IAAI,CAAE,CAAC,CACP,KAAK,CAAE,IAAI,CACX,MAAM,CAAE,IAAI,CACZ,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,IAAI,CACb,UAAU,CAAE,MAAM,CAClB,UAAU,CAAE,IAAI,CAChB,iBAAiB,CAAE,MAAM,CACrB,cAAc,CAAE,MAAM,CAClB,WAAW,CAAE,MAAM,CAC3B,gBAAgB,CAAE,MAAM,CACpB,aAAa,CAAE,MAAM,CACjB,eAAe,CAAE,MAAM,CAC/B,kBAAkB,CAAE,QAAQ,CAC5B,qBAAqB,CAAE,MAAM,CACzB,kBAAkB,CAAE,MAAM,CACtB,cAAc,CAAE,MAAM,CAC9B,gBAAgB,CAAE,YAAY,CAC1B,YAAY,CAAE,YAAY,CAC9B,mBAAmB,CAAE,IAAI,CACtB,gBAAgB,CAAE,IAAI,CACrB,eAAe,CAAE,IAAI,CACjB,WAAW,CAAE,IAAI,CACzB,gBAAgB,CAAE,KAAK,IAAI,aAAa,CAAC,CAAC,CAAC,GAAG,CAAC,CAC/C,OAAO,CAAE,IAAI,CAAC,KAAK,CAAC,KAAK,IAAI,aAAa,CAAC,CAAC,CAAC,GAAG,CAAC,CACjD,kCAAkC,CAAE,QAAQ,CACpC,0BAA0B,CAAE,QAAQ,CAC5C,OAAO,CAAE,CAAC,CACV,cAAc,CAAE,IAAI,AACxB,CAAC,AAED,MAAM,qCAAO,CAAE,aAAa,OAAO,8BAAC,CAAC,AACjC,OAAO,CAAE,CAAC,CACV,cAAc,CAAE,IAAI,AACxB,CAAC,AAED,YAAY,8BAAC,CAAC,AACV,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,IAAI,CACb,kBAAkB,CAAE,QAAQ,CAC5B,qBAAqB,CAAE,MAAM,CACzB,kBAAkB,CAAE,MAAM,CACtB,cAAc,CAAE,MAAM,CAC9B,iBAAiB,CAAE,OAAO,CACtB,cAAc,CAAE,OAAO,CACnB,WAAW,CAAE,OAAO,CAC5B,gBAAgB,CAAE,OAAO,CACrB,aAAa,CAAE,OAAO,CAClB,eAAe,CAAE,OAAO,CAChC,QAAQ,CAAE,MAAM,CAChB,gBAAgB,CAAE,KAAK,IAAI,qBAAqB,CAAC,CAAC,AACtD,CAAC,AACD,qBAAM,CAAC,YAAY,eAAC,CAAC,AACjB,KAAK,CAAE,KAAK,CACZ,aAAa,CAAE,IAAI,mBAAmB,CAAC,AAC3C,CAAC,AAED,sBAAO,CAAC,YAAY,eAAC,CAAC,AAClB,KAAK,CAAE,KAAK,KAAK,CAAC,CAAC,CAAC,IAAI,gBAAgB,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAC9C,aAAa,CAAE,IAAI,mBAAmB,CAAC,AAC3C,CAAC,AACD,mBAAI,CAAC,YAAY,eAAC,CAAC,AACf,KAAK,CAAE,KAAK,IAAI,CAAC,CAAC,CAAC,IAAI,gBAAgB,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAC7C,MAAM,CAAE,KAAK,IAAI,CAAC,CAAC,CAAC,IAAI,gBAAgB,CAAC,CAAC,CAAC,CAAC,CAAC,CAAC,CAC9C,aAAa,CAAE,IAAI,mBAAmB,CAAC,AAC3C,CAAC,AAED,KAAK,8BAAC,CAAC,AACH,iBAAiB,CAAE,OAAO,CACtB,cAAc,CAAE,OAAO,CACnB,WAAW,CAAE,OAAO,CAC5B,gBAAgB,CAAE,OAAO,CACrB,aAAa,CAAE,OAAO,CAClB,eAAe,CAAE,OAAO,AACpC,CAAC,AAED,oBAAK,CAAC,YAAY,eAAC,CAAC,AAChB,gBAAgB,CAAE,CAAC,CACf,QAAQ,CAAE,IAAI,CACV,IAAI,CAAE,IAAI,CAClB,KAAK,CAAE,IAAI,CACX,UAAU,CAAE,IAAI,CAChB,aAAa,CAAE,CAAC,AACpB,CAAC,AAED,aAAa,8BAAC,CAAC,AACX,iBAAiB,CAAE,WAAW,CAAC,CAAC,CACxB,SAAS,CAAE,WAAW,CAAC,CAAC,CAChC,OAAO,CAAE,CAAC,CACV,QAAQ,CAAE,KAAK,CACf,GAAG,CAAE,CAAC,CACN,IAAI,CAAE,CAAC,CACP,KAAK,CAAE,IAAI,CACX,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,IAAI,CACb,iBAAiB,CAAE,MAAM,CACrB,cAAc,CAAE,MAAM,CAClB,WAAW,CAAE,MAAM,CAC3B,gBAAgB,CAAE,OAAO,CACrB,aAAa,CAAE,OAAO,CAClB,eAAe,CAAE,aAAa,CACtC,KAAK,CAAE,IAAI,IAAI,aAAa,CAAC,CAAC,CAC9B,gBAAgB,CAAE,IAAI,IAAI,YAAY,CAAC,CAAC,CACxC,OAAO,CAAE,CAAC,CACV,cAAc,CAAE,IAAI,CACpB,wBAAwB,CAAE,GAAG,CAAC,IAAI,CAC1B,gBAAgB,CAAE,GAAG,CAAC,IAAI,AACtC,CAAC,AAED,4BAAa,CAAC,MAAM,eAAC,CAAC,AAClB,SAAS,CAAE,IAAI,CACf,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,IAAI,CACb,iBAAiB,CAAE,MAAM,CACrB,cAAc,CAAE,MAAM,CAClB,WAAW,CAAE,MAAM,CAC3B,gBAAgB,CAAE,MAAM,CACpB,aAAa,CAAE,MAAM,CACjB,eAAe,CAAE,MAAM,CAC/B,KAAK,CAAE,IAAI,CACX,MAAM,CAAE,IAAI,AAChB,CAAC\"}"
};
...
const Modal = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	? `${validate_component(Portal, "Portal").$$render($$result, {}, {}, {
			default: () => `
        <div${add_attribute("id", `modal-${id}`, 0)} aria-hidden="${"true"}" class="${escape(null_to_empty(classProp)) + " svelte-13jhqpw"}"${add_attribute("this", ref, 1)}>
            ${withHeader
			? `${validate_component(Portal, "Portal").$$render($$result, {}, {}, {
					default: () => `
                    ${$$slots.header
					...
					: `
                        <button type="${"button"}" class="${escape(null_to_empty(classnames("modal-header", { active }))) + " svelte-13jhqpw"}"${add_attribute("this", refHeader, 1)}>
                            <h2 style="${"padding: 15px 20px"}">Закрити</h2>
                            <span class="${"close svelte-13jhqpw"}">✕</span>
                        </button>
                    `}
                `
				})}   
			...
			: ``}
            <div class="${"modal-inner svelte-13jhqpw"}" tabindex="${"-1"}" role="${"dialog"}" aria-modal="${"true"}" aria-labelledby="${"модальне вікно"}">
                ${$$slots.default
			? $$slots.default({
					props: safeGet(() => $modals[`modal-${id}`], {}, true)
				})
...
const css$5 = {
	code: ".picture.svelte-74fc3k.svelte-74fc3k{position:relative;-webkit-box-flex:1;-ms-flex-positive:1;flex-grow:1;-ms-flex-item-align:stretch;align-self:stretch;display:-webkit-inline-box;display:-ms-inline-flexbox;display:inline-flex;-webkit-box-orient:vertical;-webkit-box-direction:normal;-ms-flex-direction:column;flex-direction:column;-webkit-box-align:stretch;-ms-flex-align:stretch;align-items:stretch;-webkit-box-pack:stretch;-ms-flex-pack:stretch;justify-content:stretch;background-color:rgba(var(--theme-bg-color-opposite), .04)}.picture.svelte-74fc3k .pic.svelte-74fc3k{-webkit-box-flex:1;-ms-flex-positive:1;flex-grow:1;overflow:hidden;-ms-flex-item-align:stretch;align-self:stretch;-o-object-position:center;object-position:center;-webkit-transition:opacity .3s ease-in;transition:opacity .3s ease-in}.picture.svelte-74fc3k .pic-2x.svelte-74fc3k{position:absolute;top:0;left:0;width:100%;height:100%}.picture.cover.svelte-74fc3k .pic.svelte-74fc3k{-o-object-fit:cover;object-fit:cover}.picture.contain.svelte-74fc3k .pic.svelte-74fc3k{-o-object-fit:contain;object-fit:contain}.picture.isError.svelte-74fc3k .pic-1x.svelte-74fc3k,.picture.isError.svelte-74fc3k .pic-2x.svelte-74fc3k,.picture.loadingSrcSmall.svelte-74fc3k .pic-1x.svelte-74fc3k,.picture.loadingSrcBig.svelte-74fc3k .pic-2x.svelte-74fc3k{opacity:0}",
	map: "{\"version\":3,\"file\":\"Picture.svelte\",\"sources\":[\"Picture.svelte\"],\"sourcesContent\":[\"<script>\\n    import { createEventDispatcher } from 'svelte'\\n    import { classnames } from '@utils'\\n\\n    const dispatch = createEventDispatcher()\\n\\n    export let src\\n    export let alt\\n    export let size = 'cover'\\n    export let srcBig = undefined\\n    export let id = undefined\\n    export let width = undefined\\n    export let height = undefined\\n\\n    let isError = false\\n    let loadingSrcSmall = true\\n    let loadingSrcBig = true\\n\\n    $: wrapClassProp = classnames('picture', $$props.class, size, { loadingSrcSmall, loadingSrcBig, isError })\\n\\n    function imgService(node, postFix) {\\n        if (node.complete) {\\n            onLoad(node, postFix)\\n        } else {\\n            node.onload = () => onLoad(node, postFix)\\n            node.onerror = () => onError(node, postFix)\\n        }\\n    }\\n\\n    function onLoad(node, postFix) {\\n        changeLoading(postFix, false)\\n        isError = false\\n        dispatch(`load${postFix}`, node)\\n\\n        if (!srcBig || !loadingSrcBig) {\\n            dispatch('load', node)\\n        }\\n    }\\n\\n    function onError(node, postFix) {\\n        changeLoading(postFix, false)\\n        isError = true\\n        dispatch(`error${postFix}`, node)\\n    }\\n\\n    function changeLoading(type, isLoading) {\\n        switch (type) {\\n            case 'Small':\\n                loadingSrcSmall = isLoading\\n                break\\n            case 'Big':\\n                loadingSrcBig = isLoading\\n                break\\n        }\\n    }\\n\\n</script>\\n\\n<figure class={wrapClassProp}>\\n    <img\\n            use:imgService={'Small'}\\n            {id}\\n            {alt}\\n            {src}\\n            {width}\\n            {height}\\n            class=\\\"pic pic-1x\\\"\\n    />\\n\\n    {#if srcBig && !loadingSrcSmall}\\n        <img\\n                use:imgService={'Big'}\\n                {alt}\\n                {width}\\n                {height}\\n                src={srcBig}\\n                class=\\\"pic pic-2x\\\"\\n        />\\n    {/if}\\n\\n    <figcaption>\\n        <slot></slot>\\n    </figcaption>\\n</figure>\\n\\n<style>\\n    .picture {\\n        position: relative;\\n        -webkit-box-flex: 1;\\n            -ms-flex-positive: 1;\\n                flex-grow: 1;\\n        -ms-flex-item-align: stretch;\\n            align-self: stretch;\\n        display: -webkit-inline-box;\\n        display: -ms-inline-flexbox;\\n        display: inline-flex;\\n        -webkit-box-orient: vertical;\\n        -webkit-box-direction: normal;\\n            -ms-flex-direction: column;\\n                flex-direction: column;\\n        -webkit-box-align: stretch;\\n            -ms-flex-align: stretch;\\n                align-items: stretch;\\n        -webkit-box-pack: stretch;\\n            -ms-flex-pack: stretch;\\n                justify-content: stretch;\\n        background-color: rgba(var(--theme-bg-color-opposite), .04);\\n    }\\n\\n    .picture .pic {\\n        -webkit-box-flex: 1;\\n            -ms-flex-positive: 1;\\n                flex-grow: 1;\\n        overflow: hidden;\\n        -ms-flex-item-align: stretch;\\n            align-self: stretch;\\n        -o-object-position: center;\\n           object-position: center;\\n        -webkit-transition: opacity .3s ease-in;\\n        transition: opacity .3s ease-in;\\n    }\\n\\n    .picture .pic-2x {\\n        position: absolute;\\n        top: 0;\\n        left: 0;\\n        width: 100%;\\n        height: 100%;\\n    }\\n\\n    .picture.cover .pic {\\n        -o-object-fit: cover;\\n           object-fit: cover;\\n    }\\n\\n    .picture.contain .pic {\\n        -o-object-fit: contain;\\n           object-fit: contain;\\n    }\\n\\n    .picture.isError .pic-1x,\\n    .picture.isError .pic-2x,\\n    .picture.loadingSrcSmall .pic-1x,\\n    .picture.loadingSrcBig .pic-2x {\\n        opacity: 0;\\n    }\\n\\n/*# sourceMappingURL=data:application/json;base64, */</style>\\n\"],\"names\":[],\"mappings\":\"AAsFI,QAAQ,4BAAC,CAAC,AACN,QAAQ,CAAE,QAAQ,CAClB,gBAAgB,CAAE,CAAC,CACf,iBAAiB,CAAE,CAAC,CAChB,SAAS,CAAE,CAAC,CACpB,mBAAmB,CAAE,OAAO,CACxB,UAAU,CAAE,OAAO,CACvB,OAAO,CAAE,kBAAkB,CAC3B,OAAO,CAAE,kBAAkB,CAC3B,OAAO,CAAE,WAAW,CACpB,kBAAkB,CAAE,QAAQ,CAC5B,qBAAqB,CAAE,MAAM,CACzB,kBAAkB,CAAE,MAAM,CACtB,cAAc,CAAE,MAAM,CAC9B,iBAAiB,CAAE,OAAO,CACtB,cAAc,CAAE,OAAO,CACnB,WAAW,CAAE,OAAO,CAC5B,gBAAgB,CAAE,OAAO,CACrB,aAAa,CAAE,OAAO,CAClB,eAAe,CAAE,OAAO,CAChC,gBAAgB,CAAE,KAAK,IAAI,yBAAyB,CAAC,CAAC,CAAC,GAAG,CAAC,AAC/D,CAAC,AAED,sBAAQ,CAAC,IAAI,cAAC,CAAC,AACX,gBAAgB,CAAE,CAAC,CACf,iBAAiB,CAAE,CAAC,CAChB,SAAS,CAAE,CAAC,CACpB,QAAQ,CAAE,MAAM,CAChB,mBAAmB,CAAE,OAAO,CACxB,UAAU,CAAE,OAAO,CACvB,kBAAkB,CAAE,MAAM,CACvB,eAAe,CAAE,MAAM,CAC1B,kBAAkB,CAAE,OAAO,CAAC,GAAG,CAAC,OAAO,CACvC,UAAU,CAAE,OAAO,CAAC,GAAG,CAAC,OAAO,AACnC,CAAC,AAED,sBAAQ,CAAC,OAAO,cAAC,CAAC,AACd,QAAQ,CAAE,QAAQ,CAClB,GAAG,CAAE,CAAC,CACN,IAAI,CAAE,CAAC,CACP,KAAK,CAAE,IAAI,CACX,MAAM,CAAE,IAAI,AAChB,CAAC,AAED,QAAQ,oBAAM,CAAC,IAAI,cAAC,CAAC,AACjB,aAAa,CAAE,KAAK,CACjB,UAAU,CAAE,KAAK,AACxB,CAAC,AAED,QAAQ,sBAAQ,CAAC,IAAI,cAAC,CAAC,AACnB,aAAa,CAAE,OAAO,CACnB,UAAU,CAAE,OAAO,AAC1B,CAAC,AAED,QAAQ,sBAAQ,CAAC,qBAAO,CACxB,QAAQ,sBAAQ,CAAC,qBAAO,CACxB,QAAQ,8BAAgB,CAAC,qBAAO,CAChC,QAAQ,4BAAc,CAAC,OAAO,cAAC,CAAC,AAC5B,OAAO,CAAE,CAAC,AACd,CAAC\"}"
};
...
const Picture = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	return `<figure class="${escape(null_to_empty(wrapClassProp)) + " svelte-74fc3k"}">
    <img${add_attribute("id", id, 0)}${add_attribute("alt", alt, 0)}${add_attribute("src", src, 0)}${add_attribute("width", width, 0)}${add_attribute("height", height, 0)} class="${"pic pic-1x svelte-74fc3k"}">

    ${srcBig && !loadingSrcSmall
	? `<img${add_attribute("alt", alt, 0)}${add_attribute("width", width, 0)}${add_attribute("height", height, 0)}${add_attribute("src", srcBig, 0)} class="${"pic pic-2x svelte-74fc3k"}">`
	: ``}
...
const css$b = {
	code: ".carousel.rounded.svelte-grgeq3.svelte-grgeq3>.carousel-inner.svelte-grgeq3.svelte-grgeq3{border-radius:var(--border-radius-big)}.carousel.svelte-grgeq3.svelte-grgeq3.svelte-grgeq3,.carousel-inner.svelte-grgeq3.svelte-grgeq3.svelte-grgeq3,.carousel-inner.svelte-grgeq3.svelte-grgeq3>li.svelte-grgeq3.svelte-grgeq3{position:relative;-webkit-box-flex:0;-ms-flex:none;flex:none;display:-webkit-box;display:-ms-flexbox;display:flex;overflow:hidden;text-align:left;-ms-flex-item-align:stretch;align-self:stretch;-webkit-box-align:stretch;-ms-flex-align:stretch;align-items:stretch;-webkit-box-pack:stretch;-ms-flex-pack:stretch;justify-content:stretch}.carousel.dotsBelow.svelte-grgeq3.svelte-grgeq3.svelte-grgeq3{padding-bottom:40px}.carousel.dotsBelow.svelte-grgeq3.svelte-grgeq3 .carousel-dots.svelte-grgeq3.svelte-grgeq3{bottom:0}.carousel.dotsBelow.svelte-grgeq3.svelte-grgeq3 .carousel-dots li.svelte-grgeq3.svelte-grgeq3{background-color:rgba(var(--theme-bg-color-opposite))}.carousel.stretch.svelte-grgeq3>.carousel-inner.svelte-grgeq3>.fluid.svelte-grgeq3{-webkit-box-flex:0;-ms-flex:none;flex:none;width:100%}.carousel.auto.svelte-grgeq3>.carousel-inner.svelte-grgeq3>.fluid.svelte-grgeq3{width:auto}.carousel.svelte-grgeq3.svelte-grgeq3.svelte-grgeq3{width:100%}.carousel-inner.svelte-grgeq3.svelte-grgeq3.svelte-grgeq3::-webkit-scrollbar{display:none}.carousel.svelte-grgeq3.svelte-grgeq3>.carousel-inner.svelte-grgeq3.svelte-grgeq3{width:100%;overflow-y:hidden;overflow-x:scroll;background-color:rgba(var(--theme-bg-color-opposite), .04)}.carousel-dots.svelte-grgeq3.svelte-grgeq3.svelte-grgeq3{position:absolute;left:0;bottom:10px;width:100%;display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-ms-flex-align:center;align-items:center;justify-items:center;-webkit-box-pack:center;-ms-flex-pack:center;justify-content:center;pointer-events:none}.carousel-dots.svelte-grgeq3.svelte-grgeq3 li.svelte-grgeq3.svelte-grgeq3{position:relative;width:8px;height:8px;margin:5px;border-radius:50%;overflow:hidden;-webkit-box-shadow:var(--shadow-primary);box-shadow:var(--shadow-primary);background-color:rgba(var(--color-light))}.carousel-dots.svelte-grgeq3.svelte-grgeq3 li.svelte-grgeq3.svelte-grgeq3:not(.active){opacity:.5}.carousel-dots.svelte-grgeq3.svelte-grgeq3 li.active.svelte-grgeq3.svelte-grgeq3{-webkit-transform:scale(1.5);transform:scale(1.5)}",
	map: "{\"version\":3,\"file\":\"Carousel.svelte\",\"sources\":[\"Carousel.svelte\"],\"sourcesContent\":[\"<script>\\n    import { createEventDispatcher } from 'svelte'\\n    import { waitUntil, classnames } from '@utils'\\n    import Picture from '@components/Picture.svelte'\\n    import FancyBox from '@components/FancyBox.svelte'\\n\\n    const dispatch = createEventDispatcher()\\n\\n    /**\\n     *\\n     * @type {number | {\\n     *     src: string,\\n     *     srcBig: string,\\n     *     alt: string,\\n     *     onClick?: function,\\n     * }[]}\\n     */\\n    export let items = []\\n    export let dots = true\\n    export let dotsBelow = true\\n    export let rounded = true\\n    export let size = 'stretch'\\n    export let initIndex = 0\\n    export let disableFancy = false\\n    export let stopPropagation = undefined\\n\\n    let parent = null\\n\\n    $: activeDot = initIndex\\n    $: classProp = classnames('carousel', size, $$props.class, { dotsBelow, rounded })\\n    $: setScrollPosition(parent, initIndex)\\n\\n    function carousel(node) {\\n        node.ontouchstart = e => stopPropagation && (e.stopPropagation())\\n        node.ontouchmove = e => stopPropagation && (e.stopPropagation())\\n        node.ontouchend = e => stopPropagation && (e.stopPropagation())\\n\\n        parent = node\\n        setScrollPosition(node, activeDot)\\n        node.addEventListener('scroll', onScroll)\\n        return { \\n            destroy: () => {\\n                node.removeEventListener('scroll', onScroll)\\n                node.ontouchstart= null\\n                node.ontouchmove= null\\n                node.ontouchend= null\\n            }\\n        }\\n    }\\n\\n    function onScroll(e) {\\n        try {\\n            getActiveDot(e.target)\\n        } catch (err) { console.warn('Carousel does not work.', err) }\\n    }\\n\\n    function getActiveDot(parent) {\\n        const { scrollLeft, scrollWidth, offsetWidth } = parent\\n        const dotAmount = Array.from(parent.children).length\\n        const scrollX = scrollLeft / (scrollWidth - offsetWidth)\\n        const newActiveDot = Math.round(scrollX * (dotAmount - 1))\\n        if (activeDot !== newActiveDot && !isNaN(newActiveDot)) activeDot = newActiveDot\\n    }\\n\\n    function setScrollPosition(parent, activeDot) {\\n        if (!parent) return\\n        const { width } = parent.getBoundingClientRect()\\n        waitUntil(() => {\\n            parent.scrollLeft = Math.round(width * activeDot)\\n            if (parent.scrollLeft !== Math.round(width * activeDot)) {\\n              throw new Error('Not set.')\\n            }\\n        }, { interval: 50 })\\n    }\\n\\n    function onClick(item, index, e) {\\n        dispatch('click', { item, index, e })\\n        if (typeof item.onClick === 'function') item.onClick(item, index, e)\\n    }\\n\\n</script>\\n\\n<section aria-label=\\\"carousel\\\" class={classProp}>\\n    <ul \\n        use:carousel\\n        class=\\\"carousel-inner scroll-x-center\\\"\\n    >\\n        {#each items as item, index}\\n            <li class=\\\"fluid\\\" role=\\\"button\\\" on:click={onClick.bind(null, item, index)}>\\n                <slot {item} {index}>\\n                    <FancyBox disabled={disableFancy}>\\n                        <Picture key=\\\"picture\\\" {...item} alt={item.alt || 'Фото слайду'}/>\\n                        <section slot=\\\"box\\\" class=\\\"flex full-width\\\">\\n                            <Picture key=\\\"picture\\\" {...item} alt={item.alt || 'Фото слайду'}/>\\n                        </section>\\n                    </FancyBox>\\n                </slot>\\n            </li>\\n        {/each}\\n    </ul>\\n\\n\\n    {#if dots}\\n        <ul class=\\\"carousel-dots\\\">\\n            {#each items as _item, i}\\n                <li class={i === activeDot ? 'active' : ''}></li>\\n            {/each}\\n        </ul>\\n    {/if}\\n</section>\\n\\n<style>\\n    .carousel.rounded > .carousel-inner {\\n       border-radius: var(--border-radius-big);\\n    }\\n\\n    .carousel, .carousel-inner, .carousel-inner > li {\\n        position: relative;\\n        -webkit-box-flex: 0;\\n            -ms-flex: none;\\n                flex: none;\\n        display: -webkit-box;\\n        display: -ms-flexbox;\\n        display: flex;\\n        overflow: hidden;\\n        text-align: left;\\n        -ms-flex-item-align: stretch;\\n            align-self: stretch;\\n        -webkit-box-align: stretch;\\n            -ms-flex-align: stretch;\\n                align-items: stretch;\\n        -webkit-box-pack: stretch;\\n            -ms-flex-pack: stretch;\\n                justify-content: stretch;\\n    }\\n\\n    .carousel.dotsBelow {\\n        padding-bottom: 40px;\\n    }\\n\\n    .carousel.dotsBelow .carousel-dots {\\n        bottom: 0;\\n    }\\n\\n    .carousel.dotsBelow .carousel-dots li {\\n        background-color: rgba(var(--theme-bg-color-opposite));\\n    }\\n\\n    .carousel.stretch > .carousel-inner > .fluid {\\n        -webkit-box-flex: 0;\\n            -ms-flex: none;\\n                flex: none;\\n        width: 100%;\\n    }\\n\\n    .carousel.auto > .carousel-inner > .fluid {\\n        width: auto;\\n    }\\n\\n    .carousel {\\n        width: 100%;\\n    }\\n\\n    .carousel-inner::-webkit-scrollbar {\\n        display: none;\\n    }\\n\\n    .carousel > .carousel-inner {\\n        width: 100%;\\n        overflow-y: hidden;\\n        overflow-x: scroll;\\n        background-color: rgba(var(--theme-bg-color-opposite), .04);\\n    }\\n\\n    .carousel-dots {\\n        position: absolute;\\n        left: 0;\\n        bottom: 10px;\\n        width: 100%;\\n        display: -webkit-box;\\n        display: -ms-flexbox;\\n        display: flex;\\n        -webkit-box-align: center;\\n            -ms-flex-align: center;\\n                align-items: center;\\n        justify-items: center;\\n        -webkit-box-pack: center;\\n            -ms-flex-pack: center;\\n                justify-content: center;\\n        pointer-events: none;\\n    }\\n\\n    .carousel-dots li {\\n        position: relative;\\n        width: 8px;\\n        height: 8px;\\n        margin: 5px;\\n        border-radius: 50%;\\n        overflow: hidden;\\n        -webkit-box-shadow: var(--shadow-primary);\\n                box-shadow: var(--shadow-primary);\\n        background-color: rgba(var(--color-light));\\n    }\\n\\n    .carousel-dots li:not(.active) {\\n        opacity: .5;\\n    }\\n\\n    .carousel-dots li.active {\\n        -webkit-transform: scale(1.5);\\n                transform: scale(1.5);\\n    }\\n\\n/*# sourceMappingURL=data:application/json;base64, */</style>\\n\"],\"names\":[],\"mappings\":\"AAgHI,SAAS,oCAAQ,CAAG,eAAe,4BAAC,CAAC,AAClC,aAAa,CAAE,IAAI,mBAAmB,CAAC,AAC1C,CAAC,AAED,mDAAS,CAAE,yDAAe,CAAE,2CAAe,CAAG,EAAE,4BAAC,CAAC,AAC9C,QAAQ,CAAE,QAAQ,CAClB,gBAAgB,CAAE,CAAC,CACf,QAAQ,CAAE,IAAI,CACV,IAAI,CAAE,IAAI,CAClB,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,IAAI,CACb,QAAQ,CAAE,MAAM,CAChB,UAAU,CAAE,IAAI,CAChB,mBAAmB,CAAE,OAAO,CACxB,UAAU,CAAE,OAAO,CACvB,iBAAiB,CAAE,OAAO,CACtB,cAAc,CAAE,OAAO,CACnB,WAAW,CAAE,OAAO,CAC5B,gBAAgB,CAAE,OAAO,CACrB,aAAa,CAAE,OAAO,CAClB,eAAe,CAAE,OAAO,AACpC,CAAC,AAED,SAAS,UAAU,0CAAC,CAAC,AACjB,cAAc,CAAE,IAAI,AACxB,CAAC,AAED,SAAS,sCAAU,CAAC,cAAc,4BAAC,CAAC,AAChC,MAAM,CAAE,CAAC,AACb,CAAC,AAED,SAAS,sCAAU,CAAC,cAAc,CAAC,EAAE,4BAAC,CAAC,AACnC,gBAAgB,CAAE,KAAK,IAAI,yBAAyB,CAAC,CAAC,AAC1D,CAAC,AAED,SAAS,sBAAQ,CAAG,6BAAe,CAAG,MAAM,cAAC,CAAC,AAC1C,gBAAgB,CAAE,CAAC,CACf,QAAQ,CAAE,IAAI,CACV,IAAI,CAAE,IAAI,CAClB,KAAK,CAAE,IAAI,AACf,CAAC,AAED,SAAS,mBAAK,CAAG,6BAAe,CAAG,MAAM,cAAC,CAAC,AACvC,KAAK,CAAE,IAAI,AACf,CAAC,AAED,SAAS,0CAAC,CAAC,AACP,KAAK,CAAE,IAAI,AACf,CAAC,AAED,yDAAe,mBAAmB,AAAC,CAAC,AAChC,OAAO,CAAE,IAAI,AACjB,CAAC,AAED,qCAAS,CAAG,eAAe,4BAAC,CAAC,AACzB,KAAK,CAAE,IAAI,CACX,UAAU,CAAE,MAAM,CAClB,UAAU,CAAE,MAAM,CAClB,gBAAgB,CAAE,KAAK,IAAI,yBAAyB,CAAC,CAAC,CAAC,GAAG,CAAC,AAC/D,CAAC,AAED,cAAc,0CAAC,CAAC,AACZ,QAAQ,CAAE,QAAQ,CAClB,IAAI,CAAE,CAAC,CACP,MAAM,CAAE,IAAI,CACZ,KAAK,CAAE,IAAI,CACX,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,IAAI,CACb,iBAAiB,CAAE,MAAM,CACrB,cAAc,CAAE,MAAM,CAClB,WAAW,CAAE,MAAM,CAC3B,aAAa,CAAE,MAAM,CACrB,gBAAgB,CAAE,MAAM,CACpB,aAAa,CAAE,MAAM,CACjB,eAAe,CAAE,MAAM,CAC/B,cAAc,CAAE,IAAI,AACxB,CAAC,AAED,0CAAc,CAAC,EAAE,4BAAC,CAAC,AACf,QAAQ,CAAE,QAAQ,CAClB,KAAK,CAAE,GAAG,CACV,MAAM,CAAE,GAAG,CACX,MAAM,CAAE,GAAG,CACX,aAAa,CAAE,GAAG,CAClB,QAAQ,CAAE,MAAM,CAChB,kBAAkB,CAAE,IAAI,gBAAgB,CAAC,CACjC,UAAU,CAAE,IAAI,gBAAgB,CAAC,CACzC,gBAAgB,CAAE,KAAK,IAAI,aAAa,CAAC,CAAC,AAC9C,CAAC,AAED,0CAAc,CAAC,8BAAE,KAAK,OAAO,CAAC,AAAC,CAAC,AAC5B,OAAO,CAAE,EAAE,AACf,CAAC,AAED,0CAAc,CAAC,EAAE,OAAO,4BAAC,CAAC,AACtB,iBAAiB,CAAE,MAAM,GAAG,CAAC,CACrB,SAAS,CAAE,MAAM,GAAG,CAAC,AACjC,CAAC\"}"
};
...
const Carousel = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	let classProp = classnames("carousel", size, $$props.class, { dotsBelow, rounded });

	 {
		setScrollPosition(parent, initIndex);
	}

	return `<section aria-label="${"carousel"}" class="${escape(null_to_empty(classProp)) + " svelte-grgeq3"}">
    <ul class="${"carousel-inner scroll-x-center svelte-grgeq3"}">
        ${each(items, (item, index) => `<li class="${"fluid svelte-grgeq3"}" role="${"button"}">
                ${$$slots.default
    ...
    ${dots
	? `<ul class="${"carousel-dots svelte-grgeq3"}">
            ${each(items, (_item, i) => `<li class="${escape(null_to_empty(i === activeDot ? "active" : "")) + " svelte-grgeq3"}"></li>`)}
        </ul>`
	: ``}
</section>`;
});
...
const Text = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	return `<rect x="${"0"}" y="${"20%"}" rx="${"3"}" ry="${"3"}" width="${"100%"}" height="${"60%"}"></rect>`;
});

/* src/components/loader/Loader.svelte generated by Svelte v3.18.1 */

const css$o = {
	code: ".loader.svelte-dc578z.svelte-dc578z{display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-flex:1;-ms-flex:1 1 auto;flex:1 1 auto;-ms-flex-item-align:stretch;align-self:stretch}.loader.svelte-dc578z svg.svelte-dc578z{-webkit-box-flex:1;-ms-flex:1 1 auto;flex:1 1 auto;-ms-flex-item-align:stretch;align-self:stretch}.loader.border.svelte-dc578z.svelte-dc578z{outline-width:1px;outline-style:solid}",
	map: "{\"version\":3,\"file\":\"Loader.svelte\",\"sources\":[\"Loader.svelte\"],\"sourcesContent\":[\"<script>\\n    // How to make a custom loader?\\n    // See here for generating: https://danilowoz.com/create-content-loader/\\n    \\n    import { onMount } from 'svelte'\\n    import { classnames } from '@utils'\\n    import Text from './Text.svelte'\\n\\n    export let width = '100%' \\n    export let height = '100%'\\n    export let light = '#999999';\\n    export let dark = '#555555';\\n    export let opacity = .2;\\n    export let border = false;\\n    export let type = 'p'; // h1, h2, h3, h4, h5, h6, p, pre\\n\\n    let hTypes = {\\n        p: 23,\\n        h1: 37,\\n        h2: 29,\\n        h3: 23,\\n        h4: 23,\\n        h5: 23,\\n        h6: 23,\\n        pre: 20,\\n    }\\n\\n    onMount(() => {\\n        const style = getComputedStyle(document.body);\\n        console.log(style.getPropertyValue('font-size'));\\n        console.log(style.getPropertyValue('line-height'));\\n    })\\n\\n    $: areaWidth = width.replace('%', '')\\n    $: areaHeight = hTypes[type] || height.replace('%', '')\\n    $: classProp = classnames('loader', { border })\\n</script>\\n\\n<section \\n    class={classProp}\\n    style={`opacity: ${opacity}; outline-color: ${light};`}\\n>  \\n    <svg\\n        role=\\\"img\\\"\\n        width=\\\"100%\\\"\\n        height={areaHeight}\\n        aria-labelledby=\\\"loading-aria\\\"\\n        viewBox={`0 0 ${areaWidth} 100`}\\n        preserveAspectRatio=\\\"none\\\"\\n    >\\n        <title id=\\\"loading-aria\\\">Loading...</title>\\n        <rect\\n            x=\\\"0\\\"\\n            y=\\\"0\\\"\\n            width=\\\"100%\\\"\\n            height=\\\"100%\\\"\\n            clip-path=\\\"url(#clip-path)\\\"\\n            style='fill: url(\\\"#fill\\\");'\\n        ></rect>\\n        <defs>\\n            <clipPath id=\\\"clip-path\\\">\\n                <slot>\\n                    {#if type}\\n                        <Text/>\\n                    {/if}\\n                </slot>\\n            </clipPath>\\n            <linearGradient id=\\\"fill\\\">\\n            <stop\\n                offset=\\\"0.599964\\\"\\n                stop-color={light}\\n                stop-opacity=\\\"1\\\"\\n            >\\n                <animate\\n                attributeName=\\\"offset\\\"\\n                values=\\\"-2; -2; 1\\\"\\n                keyTimes=\\\"0; 0.25; 1\\\"\\n                dur=\\\"2s\\\"\\n                repeatCount=\\\"indefinite\\\"\\n                ></animate>\\n            </stop>\\n            <stop\\n                offset=\\\"1.59996\\\"\\n                stop-color={dark}\\n                stop-opacity=\\\"1\\\"\\n            >\\n                <animate\\n                attributeName=\\\"offset\\\"\\n                values=\\\"-1; -1; 2\\\"\\n                keyTimes=\\\"0; 0.25; 1\\\"\\n                dur=\\\"2s\\\"\\n                repeatCount=\\\"indefinite\\\"\\n                ></animate>\\n            </stop>\\n            <stop\\n                offset=\\\"2.59996\\\"\\n                stop-color={light}\\n                stop-opacity=\\\"1\\\"\\n            >\\n                <animate\\n                attributeName=\\\"offset\\\"\\n                values=\\\"0; 0; 3\\\"\\n                keyTimes=\\\"0; 0.25; 1\\\"\\n                dur=\\\"2s\\\"\\n                repeatCount=\\\"indefinite\\\"\\n                ></animate>\\n            </stop>\\n            </linearGradient>\\n        </defs>\\n    </svg>\\n</section>\\n\\n<style>\\n    .loader {\\n        display: -webkit-box;\\n        display: -ms-flexbox;\\n        display: flex;\\n        -webkit-box-flex: 1;\\n            -ms-flex: 1 1 auto;\\n                flex: 1 1 auto;\\n        -ms-flex-item-align: stretch;\\n            align-self: stretch;\\n    }\\n\\n    .loader svg {\\n        -webkit-box-flex: 1;\\n            -ms-flex: 1 1 auto;\\n                flex: 1 1 auto;\\n        -ms-flex-item-align: stretch;\\n            align-self: stretch;\\n    }\\n\\n    .loader.border {\\n        outline-width: 1px;\\n        outline-style: solid;\\n    }\\n\\n/*# sourceMappingURL=data:application/json;base64, */</style>\\n\"],\"names\":[],\"mappings\":\"AAiHI,OAAO,4BAAC,CAAC,AACL,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,WAAW,CACpB,OAAO,CAAE,IAAI,CACb,gBAAgB,CAAE,CAAC,CACf,QAAQ,CAAE,CAAC,CAAC,CAAC,CAAC,IAAI,CACd,IAAI,CAAE,CAAC,CAAC,CAAC,CAAC,IAAI,CACtB,mBAAmB,CAAE,OAAO,CACxB,UAAU,CAAE,OAAO,AAC3B,CAAC,AAED,qBAAO,CAAC,GAAG,cAAC,CAAC,AACT,gBAAgB,CAAE,CAAC,CACf,QAAQ,CAAE,CAAC,CAAC,CAAC,CAAC,IAAI,CACd,IAAI,CAAE,CAAC,CAAC,CAAC,CAAC,IAAI,CACtB,mBAAmB,CAAE,OAAO,CACxB,UAAU,CAAE,OAAO,AAC3B,CAAC,AAED,OAAO,OAAO,4BAAC,CAAC,AACZ,aAAa,CAAE,GAAG,CAClB,aAAa,CAAE,KAAK,AACxB,CAAC\"}"
};
...
const Loader = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	let { type = "p" } = $$props; // h1, h2, h3, h4, h5, h6, p, pre
	...
	let areaHeight = hTypes[type] || height.replace("%", "");
	let classProp = classnames("loader", { border });

	return `<section class="${escape(null_to_empty(classProp)) + " svelte-dc578z"}"${add_attribute("style", `opacity: ${opacity}; outline-color: ${light};`, 0)}>  
    <svg role="${"img"}" width="${"100%"}"${add_attribute("height", areaHeight, 0)} aria-labelledby="${"loading-aria"}"${add_attribute("viewBox", `0 0 ${areaWidth} 100`, 0)} preserveAspectRatio="${"none"}" class="${"svelte-dc578z"}">
        <title id="${"loading-aria"}">Loading...</title>
