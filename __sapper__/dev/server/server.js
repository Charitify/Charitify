...
const ProgressNext = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	return `<div${add_attribute("id", id, 0)} class="${escape(null_to_empty(classProp)) + " svelte-10xvwk7"}"${add_attribute("title", titleProp, 0)}${add_attribute("aria-label", ariaLabelProp, 0)} role="${"progressbar"}" aria-valuemin="${"0"}" aria-valuemax="${"100"}"${add_attribute("aria-valuenow", val, 0)}>
    <div class="${"progress-inner-frame svelte-10xvwk7"}">
        ${each(arr, i => `<div class="${"progress-core svelte-10xvwk7"}"></div>`)}
    </div>
</div>`;
});
