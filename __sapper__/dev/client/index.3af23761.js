import { S as SvelteComponentDev, i as init, d as dispatch_dev, s as safe_not_equal, F as space, r as element, K as create_component, I as text, G as claim_space, c as claim_element, f as children, L as claim_component, g as detach_dev, J as claim_text, h as attr_dev, j as add_location, H as set_style, l as insert_dev, M as mount_component, m as append_dev, B as transition_in, z as transition_out, N as destroy_component } from './index.f49bbbed.js';
import { I as Icon, P as Picture, B as Button, a as Input, b as Progress } from './index.b7b924af.js';

/* src/routes/index.svelte generated by Svelte v3.16.7 */
const file = "src/routes/index.svelte";

// (60:16) <Button is="success" on:click="{e => console.log(e)}">
function create_default_slot_15(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test1");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test1");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_15.name,
		type: "slot",
		source: "(60:16) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\">",
		ctx
	});

	return block;
}

// (63:16) <Button is="success" on:click="{e => console.log(e)}">
function create_default_slot_14(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test12");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test12");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_14.name,
		type: "slot",
		source: "(63:16) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\">",
		ctx
	});

	return block;
}

// (66:16) <Button is="success" on:click="{e => console.log(e)}">
function create_default_slot_13(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test123");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test123");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_13.name,
		type: "slot",
		source: "(66:16) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\">",
		ctx
	});

	return block;
}

// (164:8) <Button on:click="{e => console.log(e)}">
function create_default_slot_12(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_12.name,
		type: "slot",
		source: "(164:8) <Button on:click=\\\"{e => console.log(e)}\\\">",
		ctx
	});

	return block;
}

// (167:8) <Button is="success" on:click="{e => console.log(e)}">
function create_default_slot_11(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_11.name,
		type: "slot",
		source: "(167:8) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\">",
		ctx
	});

	return block;
}

// (170:8) <Button is="success" on:click="{e => console.log(e)}" ariaLabel="text">
function create_default_slot_10(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_10.name,
		type: "slot",
		source: "(170:8) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\" ariaLabel=\\\"text\\\">",
		ctx
	});

	return block;
}

// (174:8) <Button is="success" on:click="{e => console.log(e)}">
function create_default_slot_9(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_9.name,
		type: "slot",
		source: "(174:8) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\">",
		ctx
	});

	return block;
}

// (178:8) <Button is="warning" on:click="{e => console.log(e)}">
function create_default_slot_8(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("Some example of Button");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "Some example of Button");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_8.name,
		type: "slot",
		source: "(178:8) <Button is=\\\"warning\\\" on:click=\\\"{e => console.log(e)}\\\">",
		ctx
	});

	return block;
}

// (181:8) <Button is="danger" on:click="{e => console.log(e)}">
function create_default_slot_7(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("Some example of Button");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "Some example of Button");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_7.name,
		type: "slot",
		source: "(181:8) <Button is=\\\"danger\\\" on:click=\\\"{e => console.log(e)}\\\">",
		ctx
	});

	return block;
}

// (191:8) <Button on:click="{e => console.log(e)}" href="#">
function create_default_slot_6(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_6.name,
		type: "slot",
		source: "(191:8) <Button on:click=\\\"{e => console.log(e)}\\\" href=\\\"#\\\">",
		ctx
	});

	return block;
}

// (194:8) <Button is="success" on:click="{e => console.log(e)}" href="#">
function create_default_slot_5(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_5.name,
		type: "slot",
		source: "(194:8) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\" href=\\\"#\\\">",
		ctx
	});

	return block;
}

// (197:8) <Button is="success" on:click="{e => console.log(e)}" href="#">
function create_default_slot_4(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_4.name,
		type: "slot",
		source: "(197:8) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\" href=\\\"#\\\">",
		ctx
	});

	return block;
}

// (201:8) <Button is="success" on:click="{e => console.log(e)}" href="#">
function create_default_slot_3(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("test");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "test");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_3.name,
		type: "slot",
		source: "(201:8) <Button is=\\\"success\\\" on:click=\\\"{e => console.log(e)}\\\" href=\\\"#\\\">",
		ctx
	});

	return block;
}

// (205:8) <Button is="warning" on:click="{e => console.log(e)}" href="#">
function create_default_slot_2(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("Some example of Button");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "Some example of Button");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_2.name,
		type: "slot",
		source: "(205:8) <Button is=\\\"warning\\\" on:click=\\\"{e => console.log(e)}\\\" href=\\\"#\\\">",
		ctx
	});

	return block;
}

// (208:8) <Button is="danger" on:click="{e => console.log(e)}" href="#">
function create_default_slot_1(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("Some example of Button");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "Some example of Button");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot_1.name,
		type: "slot",
		source: "(208:8) <Button is=\\\"danger\\\" on:click=\\\"{e => console.log(e)}\\\" href=\\\"#\\\">",
		ctx
	});

	return block;
}

// (260:0) <Picture alt="Borat" src="great-success.png" class="picture">
function create_default_slot(ctx) {
	let t;

	const block = {
		c: function create() {
			t = text("HIGH FIVE!");
		},
		l: function claim(nodes) {
			t = claim_text(nodes, "HIGH FIVE!");
		},
		m: function mount(target, anchor) {
			insert_dev(target, t, anchor);
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_default_slot.name,
		type: "slot",
		source: "(260:0) <Picture alt=\\\"Borat\\\" src=\\\"great-success.png\\\" class=\\\"picture\\\">",
		ctx
	});

	return block;
}

function create_fragment(ctx) {
	let t0;
	let section2;
	let t1;
	let section0;
	let div0;
	let t2;
	let ul;
	let li0;
	let t3;
	let li1;
	let t4;
	let li2;
	let t5;
	let li3;
	let t6;
	let datalist0;
	let option0;
	let option1;
	let option2;
	let t7;
	let section1;
	let t8;
	let section3;
	let br0;
	let t9;
	let br1;
	let t10;
	let t11;
	let br2;
	let t12;
	let br3;
	let t13;
	let t14;
	let br4;
	let t15;
	let br5;
	let t16;
	let t17;
	let br6;
	let t18;
	let br7;
	let t19;
	let t20;
	let br8;
	let t21;
	let br9;
	let t22;
	let t23;
	let br10;
	let t24;
	let br11;
	let t25;
	let t26;
	let br12;
	let t27;
	let br13;
	let t28;
	let t29;
	let br14;
	let t30;
	let br15;
	let t31;
	let t32;
	let h1;
	let t33;
	let span;
	let t34;
	let t35;
	let t36;
	let br16;
	let t37;
	let br17;
	let t38;
	let h20;
	let t39;
	let t40;
	let br18;
	let t41;
	let br19;
	let t42;
	let p0;
	let t43;
	let t44;
	let p1;
	let t45;
	let t46;
	let p2;
	let t47;
	let t48;
	let br20;
	let t49;
	let br21;
	let t50;
	let h21;
	let t51;
	let t52;
	let br22;
	let t53;
	let br23;
	let t54;
	let section4;
	let div1;
	let br24;
	let t55;
	let br25;
	let t56;
	let br26;
	let t57;
	let t58;
	let br27;
	let t59;
	let br28;
	let t60;
	let t61;
	let br29;
	let t62;
	let br30;
	let t63;
	let t64;
	let br31;
	let t65;
	let br32;
	let t66;
	let t67;
	let br33;
	let t68;
	let br34;
	let t69;
	let t70;
	let br35;
	let t71;
	let br36;
	let t72;
	let t73;
	let br37;
	let t74;
	let br38;
	let t75;
	let div2;
	let br39;
	let t76;
	let br40;
	let t77;
	let br41;
	let t78;
	let t79;
	let br42;
	let t80;
	let br43;
	let t81;
	let t82;
	let br44;
	let t83;
	let br45;
	let t84;
	let t85;
	let br46;
	let t86;
	let br47;
	let t87;
	let t88;
	let br48;
	let t89;
	let br49;
	let t90;
	let t91;
	let br50;
	let t92;
	let br51;
	let t93;
	let t94;
	let br52;
	let t95;
	let br53;
	let t96;
	let br54;
	let t97;
	let br55;
	let t98;
	let h22;
	let t99;
	let t100;
	let br56;
	let t101;
	let br57;
	let t102;
	let section5;
	let t103;
	let br58;
	let t104;
	let br59;
	let t105;
	let t106;
	let br60;
	let t107;
	let br61;
	let t108;
	let t109;
	let datalist1;
	let option3;
	let option4;
	let option5;
	let t110;
	let label;
	let t111;
	let t112;
	let input12;
	let t113;
	let br62;
	let t114;
	let br63;
	let t115;
	let a;
	let t116;
	let t117;
	let br64;
	let t118;
	let br65;
	let t119;
	let button15;
	let t120;
	let t121;
	let br66;
	let t122;
	let br67;
	let t123;
	let h23;
	let t124;
	let t125;
	let t126;
	let br68;
	let t127;
	let br69;
	let t128;
	let t129;
	let br70;
	let t130;
	let br71;
	let t131;
	let h24;
	let t132;
	let t133;
	let p3;
	let strong;
	let t134;
	let t135;
	let p4;
	let t136;
	let current;

	const icon = new Icon({
			props: { type: "heard-filled" },
			$$inline: true
		});

	const picture0 = new Picture({
			props: {
				src: "https://placeimg.com/1000/1000/people",
				alt: "sample"
			},
			$$inline: true
		});

	const button0 = new Button({
			props: {
				is: "success",
				$$slots: { default: [create_default_slot_15] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button0.$on("click", /*click_handler*/ ctx[0]);

	const button1 = new Button({
			props: {
				is: "success",
				$$slots: { default: [create_default_slot_14] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button1.$on("click", /*click_handler_1*/ ctx[1]);

	const button2 = new Button({
			props: {
				is: "success",
				$$slots: { default: [create_default_slot_13] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button2.$on("click", /*click_handler_2*/ ctx[2]);

	const input0 = new Input({
			props: {
				type: "number",
				name: "num",
				list: "sum-suggestions",
				placeholder: "Num",
				autoselect: true,
				align: "right"
			},
			$$inline: true
		});

	const progress = new Progress({
			props: {
				value: "65",
				max: "100",
				style: "background: 0; width: 100%"
			},
			$$inline: true
		});

	const input1 = new Input({
			props: { type: "number", list: "sum-suggestions" },
			$$inline: true
		});

	const input2 = new Input({
			props: { type: "text", list: "sum-suggestions" },
			$$inline: true
		});

	const input3 = new Input({
			props: { type: "email", list: "sum-suggestions" },
			$$inline: true
		});

	const input4 = new Input({
			props: { type: "url", list: "sum-suggestions" },
			$$inline: true
		});

	const input5 = new Input({
			props: { type: "search", list: "sum-suggestions" },
			$$inline: true
		});

	const input6 = new Input({
			props: { type: "date", list: "sum-suggestions" },
			$$inline: true
		});

	const input7 = new Input({
			props: {
				type: "password",
				list: "sum-suggestions"
			},
			$$inline: true
		});

	const input8 = new Input({
			props: { type: "tel", list: "sum-suggestions" },
			$$inline: true
		});

	const button3 = new Button({
			props: {
				$$slots: { default: [create_default_slot_12] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button3.$on("click", /*click_handler_3*/ ctx[3]);

	const button4 = new Button({
			props: {
				is: "success",
				$$slots: { default: [create_default_slot_11] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button4.$on("click", /*click_handler_4*/ ctx[4]);

	const button5 = new Button({
			props: {
				is: "success",
				ariaLabel: "text",
				$$slots: { default: [create_default_slot_10] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button5.$on("click", /*click_handler_5*/ ctx[5]);

	const button6 = new Button({
			props: {
				is: "success",
				$$slots: { default: [create_default_slot_9] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button6.$on("click", /*click_handler_6*/ ctx[6]);

	const button7 = new Button({
			props: {
				is: "warning",
				$$slots: { default: [create_default_slot_8] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button7.$on("click", /*click_handler_7*/ ctx[7]);

	const button8 = new Button({
			props: {
				is: "danger",
				$$slots: { default: [create_default_slot_7] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button8.$on("click", /*click_handler_8*/ ctx[8]);

	const button9 = new Button({
			props: {
				href: "#",
				$$slots: { default: [create_default_slot_6] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button9.$on("click", /*click_handler_9*/ ctx[9]);

	const button10 = new Button({
			props: {
				is: "success",
				href: "#",
				$$slots: { default: [create_default_slot_5] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button10.$on("click", /*click_handler_10*/ ctx[10]);

	const button11 = new Button({
			props: {
				is: "success",
				href: "#",
				$$slots: { default: [create_default_slot_4] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button11.$on("click", /*click_handler_11*/ ctx[11]);

	const button12 = new Button({
			props: {
				is: "success",
				href: "#",
				$$slots: { default: [create_default_slot_3] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button12.$on("click", /*click_handler_12*/ ctx[12]);

	const button13 = new Button({
			props: {
				is: "warning",
				href: "#",
				$$slots: { default: [create_default_slot_2] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button13.$on("click", /*click_handler_13*/ ctx[13]);

	const button14 = new Button({
			props: {
				is: "danger",
				href: "#",
				$$slots: { default: [create_default_slot_1] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	button14.$on("click", /*click_handler_14*/ ctx[14]);

	const input9 = new Input({
			props: { rows: 2, name: "tex", list: "lis" },
			$$inline: true
		});

	const input10 = new Input({
			props: {
				name: "tex1",
				list: "lis",
				placeholder: "Some placeholder text"
			},
			$$inline: true
		});

	const input11 = new Input({
			props: {
				type: "number",
				name: "tex1",
				list: "lis"
			},
			$$inline: true
		});

	const picture1 = new Picture({
			props: {
				alt: "Borat",
				src: "great-success.png",
				class: "picture",
				$$slots: { default: [create_default_slot] },
				$$scope: { ctx }
			},
			$$inline: true
		});

	const picture2 = new Picture({
			props: {
				src: "https://placeimg.com/1000/1000/any",
				alt: "sample"
			},
			$$inline: true
		});

	const block = {
		c: function create() {
			t0 = space();
			section2 = element("section");
			create_component(icon.$$.fragment);
			t1 = space();
			section0 = element("section");
			div0 = element("div");
			create_component(picture0.$$.fragment);
			t2 = space();
			ul = element("ul");
			li0 = element("li");
			create_component(button0.$$.fragment);
			t3 = space();
			li1 = element("li");
			create_component(button1.$$.fragment);
			t4 = space();
			li2 = element("li");
			create_component(button2.$$.fragment);
			t5 = space();
			li3 = element("li");
			create_component(input0.$$.fragment);
			t6 = space();
			datalist0 = element("datalist");
			option0 = element("option");
			option1 = element("option");
			option2 = element("option");
			t7 = space();
			section1 = element("section");
			create_component(progress.$$.fragment);
			t8 = space();
			section3 = element("section");
			br0 = element("br");
			t9 = space();
			br1 = element("br");
			t10 = space();
			create_component(input1.$$.fragment);
			t11 = space();
			br2 = element("br");
			t12 = space();
			br3 = element("br");
			t13 = space();
			create_component(input2.$$.fragment);
			t14 = space();
			br4 = element("br");
			t15 = space();
			br5 = element("br");
			t16 = space();
			create_component(input3.$$.fragment);
			t17 = space();
			br6 = element("br");
			t18 = space();
			br7 = element("br");
			t19 = space();
			create_component(input4.$$.fragment);
			t20 = space();
			br8 = element("br");
			t21 = space();
			br9 = element("br");
			t22 = space();
			create_component(input5.$$.fragment);
			t23 = space();
			br10 = element("br");
			t24 = space();
			br11 = element("br");
			t25 = space();
			create_component(input6.$$.fragment);
			t26 = space();
			br12 = element("br");
			t27 = space();
			br13 = element("br");
			t28 = space();
			create_component(input7.$$.fragment);
			t29 = space();
			br14 = element("br");
			t30 = space();
			br15 = element("br");
			t31 = space();
			create_component(input8.$$.fragment);
			t32 = space();
			h1 = element("h1");
			t33 = text("Great ");
			span = element("span");
			t34 = text("success");
			t35 = text("!");
			t36 = space();
			br16 = element("br");
			t37 = space();
			br17 = element("br");
			t38 = space();
			h20 = element("h2");
			t39 = text("Typography");
			t40 = space();
			br18 = element("br");
			t41 = space();
			br19 = element("br");
			t42 = space();
			p0 = element("p");
			t43 = text("Few paragraphs to text fonts");
			t44 = space();
			p1 = element("p");
			t45 = text("Декілька параграфів для тесту тексту");
			t46 = space();
			p2 = element("p");
			t47 = text("Несколько параграфов для теста текста");
			t48 = space();
			br20 = element("br");
			t49 = space();
			br21 = element("br");
			t50 = space();
			h21 = element("h2");
			t51 = text("Interactive elements");
			t52 = space();
			br22 = element("br");
			t53 = space();
			br23 = element("br");
			t54 = space();
			section4 = element("section");
			div1 = element("div");
			br24 = element("br");
			t55 = text("\n        Buttons\n        ");
			br25 = element("br");
			t56 = space();
			br26 = element("br");
			t57 = space();
			create_component(button3.$$.fragment);
			t58 = space();
			br27 = element("br");
			t59 = space();
			br28 = element("br");
			t60 = space();
			create_component(button4.$$.fragment);
			t61 = space();
			br29 = element("br");
			t62 = space();
			br30 = element("br");
			t63 = space();
			create_component(button5.$$.fragment);
			t64 = space();
			br31 = element("br");
			t65 = space();
			br32 = element("br");
			t66 = text("\n        With text\n        ");
			create_component(button6.$$.fragment);
			t67 = text("\n        and behind.\n        ");
			br33 = element("br");
			t68 = space();
			br34 = element("br");
			t69 = space();
			create_component(button7.$$.fragment);
			t70 = space();
			br35 = element("br");
			t71 = space();
			br36 = element("br");
			t72 = space();
			create_component(button8.$$.fragment);
			t73 = space();
			br37 = element("br");
			t74 = space();
			br38 = element("br");
			t75 = space();
			div2 = element("div");
			br39 = element("br");
			t76 = text("\n        Links as buttons\n        ");
			br40 = element("br");
			t77 = space();
			br41 = element("br");
			t78 = space();
			create_component(button9.$$.fragment);
			t79 = space();
			br42 = element("br");
			t80 = space();
			br43 = element("br");
			t81 = space();
			create_component(button10.$$.fragment);
			t82 = space();
			br44 = element("br");
			t83 = space();
			br45 = element("br");
			t84 = space();
			create_component(button11.$$.fragment);
			t85 = space();
			br46 = element("br");
			t86 = space();
			br47 = element("br");
			t87 = text("\n        With text\n        ");
			create_component(button12.$$.fragment);
			t88 = text("\n        and behind.\n        ");
			br48 = element("br");
			t89 = space();
			br49 = element("br");
			t90 = space();
			create_component(button13.$$.fragment);
			t91 = space();
			br50 = element("br");
			t92 = space();
			br51 = element("br");
			t93 = space();
			create_component(button14.$$.fragment);
			t94 = space();
			br52 = element("br");
			t95 = space();
			br53 = element("br");
			t96 = space();
			br54 = element("br");
			t97 = space();
			br55 = element("br");
			t98 = space();
			h22 = element("h2");
			t99 = text("Input fields");
			t100 = space();
			br56 = element("br");
			t101 = space();
			br57 = element("br");
			t102 = space();
			section5 = element("section");
			create_component(input9.$$.fragment);
			t103 = space();
			br58 = element("br");
			t104 = space();
			br59 = element("br");
			t105 = space();
			create_component(input10.$$.fragment);
			t106 = space();
			br60 = element("br");
			t107 = space();
			br61 = element("br");
			t108 = space();
			create_component(input11.$$.fragment);
			t109 = space();
			datalist1 = element("datalist");
			option3 = element("option");
			option4 = element("option");
			option5 = element("option");
			t110 = space();
			label = element("label");
			t111 = text("label");
			t112 = space();
			input12 = element("input");
			t113 = space();
			br62 = element("br");
			t114 = space();
			br63 = element("br");
			t115 = space();
			a = element("a");
			t116 = text("Some link");
			t117 = space();
			br64 = element("br");
			t118 = space();
			br65 = element("br");
			t119 = space();
			button15 = element("button");
			t120 = text("Button example");
			t121 = space();
			br66 = element("br");
			t122 = space();
			br67 = element("br");
			t123 = space();
			h23 = element("h2");
			t124 = text("Images");
			t125 = space();
			create_component(picture1.$$.fragment);
			t126 = space();
			br68 = element("br");
			t127 = space();
			br69 = element("br");
			t128 = space();
			create_component(picture2.$$.fragment);
			t129 = space();
			br70 = element("br");
			t130 = space();
			br71 = element("br");
			t131 = space();
			h24 = element("h2");
			t132 = text("Other");
			t133 = space();
			p3 = element("p");
			strong = element("strong");
			t134 = text("Try editing this file (src/routes/index.svelte) to test live reloading.");
			t135 = space();
			p4 = element("p");
			t136 = text("Lorem ipsum dolor sit amet, consectetur adipisicing elit. Autem doloribus id laborum minima. Adipisci aperiam atque\n    autem commodi consequuntur debitis distinctio dolorem eum expedita ipsam iure labore laudantium maiores mollitia\n    nesciunt\n    nostrum optio perferendis porro, possimus quas quasi quisquam sint sunt totam unde. A atque blanditiis consequatur,\n    dolore\n    ducimus et id ipsam iusto labore, laudantium libero minus nam nostrum, numquam praesentium quisquam sint?\n    Consectetur dolorum\n    enim ex ipsum nam necessitatibus nisi nostrum odio omnis, quia recusandae temporibus totam. A aliquam, assumenda aut\n    autem\n    commodi consectetur dignissimos ducimus eius eos error ipsa ipsam molestias mollitia numquam praesentium quod rem\n    sapiente\n    sed sint totam! Ab aspernatur at culpa debitis dolores ea eius, excepturi ipsa laborum, minus numquam, optio\n    reiciendis\n    repudiandae similique temporibus! Blanditiis consequatur, dignissimos dolores eaque excepturi hic inventore ipsa\n    iste\n    nemo obcaecati provident quia, quibusdam ratione reiciendis saepe sequi vel veritatis vitae voluptate voluptatem.\n    Debitis\n    illum quaerat velit voluptatibus. A accusamus ad adipisci animi, aperiam architecto aspernatur aut beatae corporis\n    culpa\n    debitis dicta distinctio dolorem ducimus ea eligendi eos eveniet excepturi harum ipsum magnam maxime nesciunt\n    numquam obcaecati\n    perferendis, provident quaerat qui ratione reiciendis repellendus reprehenderit saepe sequi vel voluptas voluptate\n    voluptates\n    voluptatibus? Amet aperiam corporis deleniti deserunt, dolorem est exercitationem fuga hic impedit inventore itaque\n    laboriosam\n    laudantium, magnam necessitatibus nemo odio officiis perferendis perspiciatis quae qui quibusdam quis quod saepe sit\n    suscipit\n    tenetur ut vel veritatis voluptate voluptatibus. Aliquid cum cumque eum iure laboriosam natus rem. Accusantium alias\n    amet\n    corporis eaque enim explicabo fuga harum illo inventore itaque laudantium modi nemo nihil nulla possimus quae quas\n    quia quidem\n    quisquam recusandae repellendus sapiente sequi sit tempora, totam unde velit. Accusantium corporis cum dolor dolorum\n    iusto\n    laboriosam minus necessitatibus, nulla, omnis pariatur quae ullam voluptatem. Accusantium architecto, assumenda\n    commodi cumque\n    dignissimos doloribus eius et eum in iusto libero magnam, magni mollitia necessitatibus neque nesciunt perspiciatis\n    quidem\n    voluptas voluptatem voluptatibus? Aspernatur assumenda at consequatur deleniti eum expedita fuga in neque officia\n    pariatur,\n    provident quasi saepe sed sequi tempora tempore voluptates? Aliquam aperiam autem cum doloribus ducimus enim eum ex\n    expedita\n    explicabo id illo iste iure maiores modi nemo neque nisi nostrum officia officiis omnis perferendis perspiciatis\n    possimus\n    quae quas quibusdam quidem quis saepe sed sint sunt velit, vero vitae voluptatum? Amet debitis, dolore dolores eum\n    in ipsum\n    magnam mollitia nostrum quas quisquam quod rem sed sequi tenetur, voluptatibus. Ad adipisci aliquam animi aut autem\n    commodi\n    distinctio dolor eveniet impedit in iusto magni neque, odit optio pariatur placeat quaerat quo rem rerum sequi\n    suscipit\n    veniam vero voluptate voluptatem, voluptatum. Accusantium dicta dolores ea est libero rerum sapiente sit!\n    Accusantium cupiditate\n    eum ex molestiae praesentium provident saepe? Eveniet labore, rem. Aperiam, architecto at consectetur culpa cumque\n    debitis\n    dolorum eius eveniet fuga illo, in ipsum libero molestias necessitatibus nemo neque omnis perferendis perspiciatis\n    praesentium\n    quasi qui quia quod rerum saepe sed similique sunt suscipit tenetur totam ullam veritatis vitae voluptate\n    voluptatem. Assumenda\n    consequatur corporis dolorem doloremque ea inventore iusto sequi sit ut vitae! Adipisci alias aliquid aperiam\n    architecto asperiores\n    delectus dolore eaque eius eligendi fugiat incidunt ipsam iusto laborum magni maxime nam, necessitatibus nesciunt\n    omnis placeat\n    quaerat quibusdam rem, suscipit unde vel, veritatis vero voluptatum. Animi aut fuga impedit nostrum odit officia\n    quisquam,\n    repudiandae saepe suscipit voluptates! Ab autem beatae distinctio, dolorum eligendi est excepturi facere facilis\n    illo inventore\n    iure minus nemo nesciunt quaerat quibusdam repellendus sequi sint sit ullam voluptatem. Debitis ducimus excepturi\n    impedit\n    laudantium magnam nisi possimus, quae quod repellat sed sit tempora unde. Delectus eius laudantium quas quidem\n    similique sit\n    unde voluptas voluptate? Accusamus aut distinctio dolorum earum enim illo illum incidunt iure magni natus nulla\n    officiis\n    pariatur perspiciatis possimus quasi recusandae sint voluptas, voluptatibus. Ad aliquam amet, deleniti dignissimos,\n    eius enim\n    error excepturi hic magni quae qui quisquam repellendus rerum saepe similique suscipit temporibus vel voluptate. A\n    asperiores\n    at, dolores eligendi eum facere fuga impedit incidunt ipsam labore laborum magnam omnis placeat, quam quidem quos\n    similique\n    tenetur totam, vero voluptatibus. Ab asperiores autem cum delectus dicta dolor dolorum eos ex impedit libero nam\n    natus, necessitatibus\n    Lorem ipsum dolor sit amet, consectetur adipisicing elit. Autem doloribus id laborum minima. Adipisci aperiam atque\n    autem commodi consequuntur debitis distinctio dolorem eum expedita ipsam iure labore laudantium maiores mollitia\n    nesciunt\n    nostrum optio perferendis porro, possimus quas quasi quisquam sint sunt totam unde. A atque blanditiis consequatur,\n    dolore\n    ducimus et id ipsam iusto labore, laudantium libero minus nam nostrum, numquam praesentium quisquam sint?\n    Consectetur dolorum\n    enim ex ipsum nam necessitatibus nisi nostrum odio omnis, quia recusandae temporibus totam. A aliquam, assumenda aut\n    autem\n    commodi consectetur dignissimos ducimus eius eos error ipsa ipsam molestias mollitia numquam praesentium quod rem\n    sapiente\n    sed sint totam! Ab aspernatur at culpa debitis dolores ea eius, excepturi ipsa laborum, minus numquam, optio\n    reiciendis\n    repudiandae similique temporibus! Blanditiis consequatur, dignissimos dolores eaque excepturi hic inventore ipsa\n    iste\n    nemo obcaecati provident quia, quibusdam ratione reiciendis saepe sequi vel veritatis vitae voluptate voluptatem.\n    Debitis\n    illum quaerat velit voluptatibus. A accusamus ad adipisci animi, aperiam architecto aspernatur aut beatae corporis\n    culpa\n    debitis dicta distinctio dolorem ducimus ea eligendi eos eveniet excepturi harum ipsum magnam maxime nesciunt\n    numquam obcaecati\n    perferendis, provident quaerat qui ratione reiciendis repellendus reprehenderit saepe sequi vel voluptas voluptate\n    voluptates\n    voluptatibus? Amet aperiam corporis deleniti deserunt, dolorem est exercitationem fuga hic impedit inventore itaque\n    laboriosam\n    laudantium, magnam necessitatibus nemo odio officiis perferendis perspiciatis quae qui quibusdam quis quod saepe sit\n    suscipit\n    tenetur ut vel veritatis voluptate voluptatibus. Aliquid cum cumque eum iure laboriosam natus rem. Accusantium alias\n    amet\n    corporis eaque enim explicabo fuga harum illo inventore itaque laudantium modi nemo nihil nulla possimus quae quas\n    quia quidem\n    quisquam recusandae repellendus sapiente sequi sit tempora, totam unde velit. Accusantium corporis cum dolor dolorum\n    iusto\n    laboriosam minus necessitatibus, nulla, omnis pariatur quae ullam voluptatem. Accusantium architecto, assumenda\n    commodi cumque\n    dignissimos doloribus eius et eum in iusto libero magnam, magni mollitia necessitatibus neque nesciunt perspiciatis\n    quidem\n    voluptas voluptatem voluptatibus? Aspernatur assumenda at consequatur deleniti eum expedita fuga in neque officia\n    pariatur,\n    provident quasi saepe sed sequi tempora tempore voluptates? Aliquam aperiam autem cum doloribus ducimus enim eum ex\n    expedita\n    explicabo id illo iste iure maiores modi nemo neque nisi nostrum officia officiis omnis perferendis perspiciatis\n    possimus\n    quae quas quibusdam quidem quis saepe sed sint sunt velit, vero vitae voluptatum? Amet debitis, dolore dolores eum\n    in ipsum\n    magnam mollitia nostrum quas quisquam quod rem sed sequi tenetur, voluptatibus. Ad adipisci aliquam animi aut autem\n    commodi\n    distinctio dolor eveniet impedit in iusto magni neque, odit optio pariatur placeat quaerat quo rem rerum sequi\n    suscipit\n    veniam vero voluptate voluptatem, voluptatum. Accusantium dicta dolores ea est libero rerum sapiente sit!\n    Accusantium cupiditate\n    eum ex molestiae praesentium provident saepe? Eveniet labore, rem. Aperiam, architecto at consectetur culpa cumque\n    debitis\n    dolorum eius eveniet fuga illo, in ipsum libero molestias necessitatibus nemo neque omnis perferendis perspiciatis\n    praesentium\n    quasi qui quia quod rerum saepe sed similique sunt suscipit tenetur totam ullam veritatis vitae voluptate\n    voluptatem. Assumenda\n    consequatur corporis dolorem doloremque ea inventore iusto sequi sit ut vitae! Adipisci alias aliquid aperiam\n    architecto asperiores\n    delectus dolore eaque eius eligendi fugiat incidunt ipsam iusto laborum magni maxime nam, necessitatibus nesciunt\n    omnis placeat\n    quaerat quibusdam rem, suscipit unde vel, veritatis vero voluptatum. Animi aut fuga impedit nostrum odit officia\n    quisquam,\n    repudiandae saepe suscipit voluptates! Ab autem beatae distinctio, dolorum eligendi est excepturi facere facilis\n    illo inventore\n    iure minus nemo nesciunt quaerat quibusdam repellendus sequi sint sit ullam voluptatem. Debitis ducimus excepturi\n    impedit\n    laudantium magnam nisi possimus, quae quod repellat sed sit tempora unde. Delectus eius laudantium quas quidem\n    similique sit\n    unde voluptas voluptate? Accusamus aut distinctio dolorum earum enim illo illum incidunt iure magni natus nulla\n    officiis\n    pariatur perspiciatis possimus quasi recusandae sint voluptas, voluptatibus. Ad aliquam amet, deleniti dignissimos,\n    eius enim\n    error excepturi hic magni quae qui quisquam repellendus rerum saepe similique suscipit temporibus vel voluptate. A\n    asperiores\n    at, dolores eligendi eum facere fuga impedit incidunt ipsam labore laborum magnam omnis placeat, quam quidem quos\n    similique\n    tenetur totam, vero voluptatibus. Ab asperiores autem cum delectus dicta dolor dolorum eos ex impedit libero nam\n    natus, necessitatibus\n    Lorem ipsum dolor sit amet, consectetur adipisicing elit. Autem doloribus id laborum minima. Adipisci aperiam atque\n    autem commodi consequuntur debitis distinctio dolorem eum expedita ipsam iure labore laudantium maiores mollitia\n    nesciunt\n    nostrum optio perferendis porro, possimus quas quasi quisquam sint sunt totam unde. A atque blanditiis consequatur,\n    dolore\n    ducimus et id ipsam iusto labore, laudantium libero minus nam nostrum, numquam praesentium quisquam sint?\n    Consectetur dolorum\n    enim ex ipsum nam necessitatibus nisi nostrum odio omnis, quia recusandae temporibus totam. A aliquam, assumenda aut\n    autem\n    commodi consectetur dignissimos ducimus eius eos error ipsa ipsam molestias mollitia numquam praesentium quod rem\n    sapiente\n    sed sint totam! Ab aspernatur at culpa debitis dolores ea eius, excepturi ipsa laborum, minus numquam, optio\n    reiciendis\n    repudiandae similique temporibus! Blanditiis consequatur, dignissimos dolores eaque excepturi hic inventore ipsa\n    iste\n    nemo obcaecati provident quia, quibusdam ratione reiciendis saepe sequi vel veritatis vitae voluptate voluptatem.\n    Debitis\n    illum quaerat velit voluptatibus. A accusamus ad adipisci animi, aperiam architecto aspernatur aut beatae corporis\n    culpa\n    debitis dicta distinctio dolorem ducimus ea eligendi eos eveniet excepturi harum ipsum magnam maxime nesciunt\n    numquam obcaecati\n    perferendis, provident quaerat qui ratione reiciendis repellendus reprehenderit saepe sequi vel voluptas voluptate\n    voluptates\n    voluptatibus? Amet aperiam corporis deleniti deserunt, dolorem est exercitationem fuga hic impedit inventore itaque\n    laboriosam\n    laudantium, magnam necessitatibus nemo odio officiis perferendis perspiciatis quae qui quibusdam quis quod saepe sit\n    suscipit\n    tenetur ut vel veritatis voluptate voluptatibus. Aliquid cum cumque eum iure laboriosam natus rem. Accusantium alias\n    amet\n    corporis eaque enim explicabo fuga harum illo inventore itaque laudantium modi nemo nihil nulla possimus quae quas\n    quia quidem\n    quisquam recusandae repellendus sapiente sequi sit tempora, totam unde velit. Accusantium corporis cum dolor dolorum\n    iusto\n    laboriosam minus necessitatibus, nulla, omnis pariatur quae ullam voluptatem. Accusantium architecto, assumenda\n    commodi cumque\n    dignissimos doloribus eius et eum in iusto libero magnam, magni mollitia necessitatibus neque nesciunt perspiciatis\n    quidem\n    voluptas voluptatem voluptatibus? Aspernatur assumenda at consequatur deleniti eum expedita fuga in neque officia\n    pariatur,\n    provident quasi saepe sed sequi tempora tempore voluptates? Aliquam aperiam autem cum doloribus ducimus enim eum ex\n    expedita\n    explicabo id illo iste iure maiores modi nemo neque nisi nostrum officia officiis omnis perferendis perspiciatis\n    possimus\n    quae quas quibusdam quidem quis saepe sed sint sunt velit, vero vitae voluptatum? Amet debitis, dolore dolores eum\n    in ipsum\n    magnam mollitia nostrum quas quisquam quod rem sed sequi tenetur, voluptatibus. Ad adipisci aliquam animi aut autem\n    commodi\n    distinctio dolor eveniet impedit in iusto magni neque, odit optio pariatur placeat quaerat quo rem rerum sequi\n    suscipit\n    veniam vero voluptate voluptatem, voluptatum. Accusantium dicta dolores ea est libero rerum sapiente sit!\n    Accusantium cupiditate\n    eum ex molestiae praesentium provident saepe? Eveniet labore, rem. Aperiam, architecto at consectetur culpa cumque\n    debitis\n    dolorum eius eveniet fuga illo, in ipsum libero molestias necessitatibus nemo neque omnis perferendis perspiciatis\n    praesentium\n    quasi qui quia quod rerum saepe sed similique sunt suscipit tenetur totam ullam veritatis vitae voluptate\n    voluptatem. Assumenda\n    consequatur corporis dolorem doloremque ea inventore iusto sequi sit ut vitae! Adipisci alias aliquid aperiam\n    architecto asperiores\n    delectus dolore eaque eius eligendi fugiat incidunt ipsam iusto laborum magni maxime nam, necessitatibus nesciunt\n    omnis placeat\n    quaerat quibusdam rem, suscipit unde vel, veritatis vero voluptatum. Animi aut fuga impedit nostrum odit officia\n    quisquam,\n    repudiandae saepe suscipit voluptates! Ab autem beatae distinctio, dolorum eligendi est excepturi facere facilis\n    illo inventore\n    iure minus nemo nesciunt quaerat quibusdam repellendus sequi sint sit ullam voluptatem. Debitis ducimus excepturi\n    impedit\n    laudantium magnam nisi possimus, quae quod repellat sed sit tempora unde. Delectus eius laudantium quas quidem\n    similique sit\n    unde voluptas voluptate? Accusamus aut distinctio dolorum earum enim illo illum incidunt iure magni natus nulla\n    officiis\n    pariatur perspiciatis possimus quasi recusandae sint voluptas, voluptatibus. Ad aliquam amet, deleniti dignissimos,\n    eius enim\n    error excepturi hic magni quae qui quisquam repellendus rerum saepe similique suscipit temporibus vel voluptate. A\n    asperiores\n    at, dolores eligendi eum facere fuga impedit incidunt ipsam labore laborum magnam omnis placeat, quam quidem quos\n    similique\n    tenetur totam, vero voluptatibus. Ab asperiores autem cum delectus dicta dolor dolorum eos ex impedit libero nam\n    natus, necessitatibus");
			this.h();
		},
		l: function claim(nodes) {
			t0 = claim_space(nodes);
			section2 = claim_element(nodes, "SECTION", { class: true });
			var section2_nodes = children(section2);
			claim_component(icon.$$.fragment, section2_nodes);
			t1 = claim_space(section2_nodes);
			section0 = claim_element(section2_nodes, "SECTION", { class: true });
			var section0_nodes = children(section0);
			div0 = claim_element(section0_nodes, "DIV", { class: true });
			var div0_nodes = children(div0);
			claim_component(picture0.$$.fragment, div0_nodes);
			div0_nodes.forEach(detach_dev);
			t2 = claim_space(section0_nodes);
			ul = claim_element(section0_nodes, "UL", { class: true });
			var ul_nodes = children(ul);
			li0 = claim_element(ul_nodes, "LI", { class: true });
			var li0_nodes = children(li0);
			claim_component(button0.$$.fragment, li0_nodes);
			li0_nodes.forEach(detach_dev);
			t3 = claim_space(ul_nodes);
			li1 = claim_element(ul_nodes, "LI", { class: true });
			var li1_nodes = children(li1);
			claim_component(button1.$$.fragment, li1_nodes);
			li1_nodes.forEach(detach_dev);
			t4 = claim_space(ul_nodes);
			li2 = claim_element(ul_nodes, "LI", { class: true });
			var li2_nodes = children(li2);
			claim_component(button2.$$.fragment, li2_nodes);
			li2_nodes.forEach(detach_dev);
			t5 = claim_space(ul_nodes);
			li3 = claim_element(ul_nodes, "LI", { class: true });
			var li3_nodes = children(li3);
			claim_component(input0.$$.fragment, li3_nodes);
			t6 = claim_space(li3_nodes);
			datalist0 = claim_element(li3_nodes, "DATALIST", { id: true });
			var datalist0_nodes = children(datalist0);
			option0 = claim_element(datalist0_nodes, "OPTION", { value: true });
			var option0_nodes = children(option0);
			option0_nodes.forEach(detach_dev);
			option1 = claim_element(datalist0_nodes, "OPTION", { value: true });
			var option1_nodes = children(option1);
			option1_nodes.forEach(detach_dev);
			option2 = claim_element(datalist0_nodes, "OPTION", { value: true });
			var option2_nodes = children(option2);
			option2_nodes.forEach(detach_dev);
			datalist0_nodes.forEach(detach_dev);
			li3_nodes.forEach(detach_dev);
			ul_nodes.forEach(detach_dev);
			section0_nodes.forEach(detach_dev);
			t7 = claim_space(section2_nodes);
			section1 = claim_element(section2_nodes, "SECTION", { style: true });
			var section1_nodes = children(section1);
			claim_component(progress.$$.fragment, section1_nodes);
			section1_nodes.forEach(detach_dev);
			section2_nodes.forEach(detach_dev);
			t8 = claim_space(nodes);
			section3 = claim_element(nodes, "SECTION", {});
			var section3_nodes = children(section3);
			br0 = claim_element(section3_nodes, "BR", {});
			t9 = claim_space(section3_nodes);
			br1 = claim_element(section3_nodes, "BR", {});
			t10 = claim_space(section3_nodes);
			claim_component(input1.$$.fragment, section3_nodes);
			t11 = claim_space(section3_nodes);
			br2 = claim_element(section3_nodes, "BR", {});
			t12 = claim_space(section3_nodes);
			br3 = claim_element(section3_nodes, "BR", {});
			t13 = claim_space(section3_nodes);
			claim_component(input2.$$.fragment, section3_nodes);
			t14 = claim_space(section3_nodes);
			br4 = claim_element(section3_nodes, "BR", {});
			t15 = claim_space(section3_nodes);
			br5 = claim_element(section3_nodes, "BR", {});
			t16 = claim_space(section3_nodes);
			claim_component(input3.$$.fragment, section3_nodes);
			t17 = claim_space(section3_nodes);
			br6 = claim_element(section3_nodes, "BR", {});
			t18 = claim_space(section3_nodes);
			br7 = claim_element(section3_nodes, "BR", {});
			t19 = claim_space(section3_nodes);
			claim_component(input4.$$.fragment, section3_nodes);
			t20 = claim_space(section3_nodes);
			br8 = claim_element(section3_nodes, "BR", {});
			t21 = claim_space(section3_nodes);
			br9 = claim_element(section3_nodes, "BR", {});
			t22 = claim_space(section3_nodes);
			claim_component(input5.$$.fragment, section3_nodes);
			t23 = claim_space(section3_nodes);
			br10 = claim_element(section3_nodes, "BR", {});
			t24 = claim_space(section3_nodes);
			br11 = claim_element(section3_nodes, "BR", {});
			t25 = claim_space(section3_nodes);
			claim_component(input6.$$.fragment, section3_nodes);
			t26 = claim_space(section3_nodes);
			br12 = claim_element(section3_nodes, "BR", {});
			t27 = claim_space(section3_nodes);
			br13 = claim_element(section3_nodes, "BR", {});
			t28 = claim_space(section3_nodes);
			claim_component(input7.$$.fragment, section3_nodes);
			t29 = claim_space(section3_nodes);
			br14 = claim_element(section3_nodes, "BR", {});
			t30 = claim_space(section3_nodes);
			br15 = claim_element(section3_nodes, "BR", {});
			t31 = claim_space(section3_nodes);
			claim_component(input8.$$.fragment, section3_nodes);
			section3_nodes.forEach(detach_dev);
			t32 = claim_space(nodes);
			h1 = claim_element(nodes, "H1", { class: true });
			var h1_nodes = children(h1);
			t33 = claim_text(h1_nodes, "Great ");
			span = claim_element(h1_nodes, "SPAN", { class: true });
			var span_nodes = children(span);
			t34 = claim_text(span_nodes, "success");
			span_nodes.forEach(detach_dev);
			t35 = claim_text(h1_nodes, "!");
			h1_nodes.forEach(detach_dev);
			t36 = claim_space(nodes);
			br16 = claim_element(nodes, "BR", {});
			t37 = claim_space(nodes);
			br17 = claim_element(nodes, "BR", {});
			t38 = claim_space(nodes);
			h20 = claim_element(nodes, "H2", { class: true });
			var h20_nodes = children(h20);
			t39 = claim_text(h20_nodes, "Typography");
			h20_nodes.forEach(detach_dev);
			t40 = claim_space(nodes);
			br18 = claim_element(nodes, "BR", {});
			t41 = claim_space(nodes);
			br19 = claim_element(nodes, "BR", {});
			t42 = claim_space(nodes);
			p0 = claim_element(nodes, "P", { class: true });
			var p0_nodes = children(p0);
			t43 = claim_text(p0_nodes, "Few paragraphs to text fonts");
			p0_nodes.forEach(detach_dev);
			t44 = claim_space(nodes);
			p1 = claim_element(nodes, "P", { class: true });
			var p1_nodes = children(p1);
			t45 = claim_text(p1_nodes, "Декілька параграфів для тесту тексту");
			p1_nodes.forEach(detach_dev);
			t46 = claim_space(nodes);
			p2 = claim_element(nodes, "P", { class: true });
			var p2_nodes = children(p2);
			t47 = claim_text(p2_nodes, "Несколько параграфов для теста текста");
			p2_nodes.forEach(detach_dev);
			t48 = claim_space(nodes);
			br20 = claim_element(nodes, "BR", {});
			t49 = claim_space(nodes);
			br21 = claim_element(nodes, "BR", {});
			t50 = claim_space(nodes);
			h21 = claim_element(nodes, "H2", { class: true });
			var h21_nodes = children(h21);
			t51 = claim_text(h21_nodes, "Interactive elements");
			h21_nodes.forEach(detach_dev);
			t52 = claim_space(nodes);
			br22 = claim_element(nodes, "BR", {});
			t53 = claim_space(nodes);
			br23 = claim_element(nodes, "BR", {});
			t54 = claim_space(nodes);
			section4 = claim_element(nodes, "SECTION", { style: true });
			var section4_nodes = children(section4);
			div1 = claim_element(section4_nodes, "DIV", {});
			var div1_nodes = children(div1);
			br24 = claim_element(div1_nodes, "BR", {});
			t55 = claim_text(div1_nodes, "\n        Buttons\n        ");
			br25 = claim_element(div1_nodes, "BR", {});
			t56 = claim_space(div1_nodes);
			br26 = claim_element(div1_nodes, "BR", {});
			t57 = claim_space(div1_nodes);
			claim_component(button3.$$.fragment, div1_nodes);
			t58 = claim_space(div1_nodes);
			br27 = claim_element(div1_nodes, "BR", {});
			t59 = claim_space(div1_nodes);
			br28 = claim_element(div1_nodes, "BR", {});
			t60 = claim_space(div1_nodes);
			claim_component(button4.$$.fragment, div1_nodes);
			t61 = claim_space(div1_nodes);
			br29 = claim_element(div1_nodes, "BR", {});
			t62 = claim_space(div1_nodes);
			br30 = claim_element(div1_nodes, "BR", {});
			t63 = claim_space(div1_nodes);
			claim_component(button5.$$.fragment, div1_nodes);
			t64 = claim_space(div1_nodes);
			br31 = claim_element(div1_nodes, "BR", {});
			t65 = claim_space(div1_nodes);
			br32 = claim_element(div1_nodes, "BR", {});
			t66 = claim_text(div1_nodes, "\n        With text\n        ");
			claim_component(button6.$$.fragment, div1_nodes);
			t67 = claim_text(div1_nodes, "\n        and behind.\n        ");
			br33 = claim_element(div1_nodes, "BR", {});
			t68 = claim_space(div1_nodes);
			br34 = claim_element(div1_nodes, "BR", {});
			t69 = claim_space(div1_nodes);
			claim_component(button7.$$.fragment, div1_nodes);
			t70 = claim_space(div1_nodes);
			br35 = claim_element(div1_nodes, "BR", {});
			t71 = claim_space(div1_nodes);
			br36 = claim_element(div1_nodes, "BR", {});
			t72 = claim_space(div1_nodes);
			claim_component(button8.$$.fragment, div1_nodes);
			t73 = claim_space(div1_nodes);
			br37 = claim_element(div1_nodes, "BR", {});
			t74 = claim_space(div1_nodes);
			br38 = claim_element(div1_nodes, "BR", {});
			div1_nodes.forEach(detach_dev);
			t75 = claim_space(section4_nodes);
			div2 = claim_element(section4_nodes, "DIV", {});
			var div2_nodes = children(div2);
			br39 = claim_element(div2_nodes, "BR", {});
			t76 = claim_text(div2_nodes, "\n        Links as buttons\n        ");
			br40 = claim_element(div2_nodes, "BR", {});
			t77 = claim_space(div2_nodes);
			br41 = claim_element(div2_nodes, "BR", {});
			t78 = claim_space(div2_nodes);
			claim_component(button9.$$.fragment, div2_nodes);
			t79 = claim_space(div2_nodes);
			br42 = claim_element(div2_nodes, "BR", {});
			t80 = claim_space(div2_nodes);
			br43 = claim_element(div2_nodes, "BR", {});
			t81 = claim_space(div2_nodes);
			claim_component(button10.$$.fragment, div2_nodes);
			t82 = claim_space(div2_nodes);
			br44 = claim_element(div2_nodes, "BR", {});
			t83 = claim_space(div2_nodes);
			br45 = claim_element(div2_nodes, "BR", {});
			t84 = claim_space(div2_nodes);
			claim_component(button11.$$.fragment, div2_nodes);
			t85 = claim_space(div2_nodes);
			br46 = claim_element(div2_nodes, "BR", {});
			t86 = claim_space(div2_nodes);
			br47 = claim_element(div2_nodes, "BR", {});
			t87 = claim_text(div2_nodes, "\n        With text\n        ");
			claim_component(button12.$$.fragment, div2_nodes);
			t88 = claim_text(div2_nodes, "\n        and behind.\n        ");
			br48 = claim_element(div2_nodes, "BR", {});
			t89 = claim_space(div2_nodes);
			br49 = claim_element(div2_nodes, "BR", {});
			t90 = claim_space(div2_nodes);
			claim_component(button13.$$.fragment, div2_nodes);
			t91 = claim_space(div2_nodes);
			br50 = claim_element(div2_nodes, "BR", {});
			t92 = claim_space(div2_nodes);
			br51 = claim_element(div2_nodes, "BR", {});
			t93 = claim_space(div2_nodes);
			claim_component(button14.$$.fragment, div2_nodes);
			t94 = claim_space(div2_nodes);
			br52 = claim_element(div2_nodes, "BR", {});
			t95 = claim_space(div2_nodes);
			br53 = claim_element(div2_nodes, "BR", {});
			div2_nodes.forEach(detach_dev);
			section4_nodes.forEach(detach_dev);
			t96 = claim_space(nodes);
			br54 = claim_element(nodes, "BR", {});
			t97 = claim_space(nodes);
			br55 = claim_element(nodes, "BR", {});
			t98 = claim_space(nodes);
			h22 = claim_element(nodes, "H2", { class: true });
			var h22_nodes = children(h22);
			t99 = claim_text(h22_nodes, "Input fields");
			h22_nodes.forEach(detach_dev);
			t100 = claim_space(nodes);
			br56 = claim_element(nodes, "BR", {});
			t101 = claim_space(nodes);
			br57 = claim_element(nodes, "BR", {});
			t102 = claim_space(nodes);
			section5 = claim_element(nodes, "SECTION", {});
			var section5_nodes = children(section5);
			claim_component(input9.$$.fragment, section5_nodes);
			t103 = claim_space(section5_nodes);
			br58 = claim_element(section5_nodes, "BR", {});
			t104 = claim_space(section5_nodes);
			br59 = claim_element(section5_nodes, "BR", {});
			t105 = claim_space(section5_nodes);
			claim_component(input10.$$.fragment, section5_nodes);
			t106 = claim_space(section5_nodes);
			br60 = claim_element(section5_nodes, "BR", {});
			t107 = claim_space(section5_nodes);
			br61 = claim_element(section5_nodes, "BR", {});
			t108 = claim_space(section5_nodes);
			claim_component(input11.$$.fragment, section5_nodes);
			t109 = claim_space(section5_nodes);
			datalist1 = claim_element(section5_nodes, "DATALIST", { id: true });
			var datalist1_nodes = children(datalist1);
			option3 = claim_element(datalist1_nodes, "OPTION", { value: true });
			var option3_nodes = children(option3);
			option3_nodes.forEach(detach_dev);
			option4 = claim_element(datalist1_nodes, "OPTION", { value: true });
			var option4_nodes = children(option4);
			option4_nodes.forEach(detach_dev);
			option5 = claim_element(datalist1_nodes, "OPTION", { value: true });
			var option5_nodes = children(option5);
			option5_nodes.forEach(detach_dev);
			datalist1_nodes.forEach(detach_dev);
			t110 = claim_space(section5_nodes);
			label = claim_element(section5_nodes, "LABEL", { for: true, class: true });
			var label_nodes = children(label);
			t111 = claim_text(label_nodes, "label");
			label_nodes.forEach(detach_dev);
			t112 = claim_space(section5_nodes);
			input12 = claim_element(section5_nodes, "INPUT", { id: true, type: true, value: true });
			t113 = claim_space(section5_nodes);
			br62 = claim_element(section5_nodes, "BR", {});
			t114 = claim_space(section5_nodes);
			br63 = claim_element(section5_nodes, "BR", {});
			t115 = claim_space(section5_nodes);
			a = claim_element(section5_nodes, "A", { href: true, class: true });
			var a_nodes = children(a);
			t116 = claim_text(a_nodes, "Some link");
			a_nodes.forEach(detach_dev);
			t117 = claim_space(section5_nodes);
			br64 = claim_element(section5_nodes, "BR", {});
			t118 = claim_space(section5_nodes);
			br65 = claim_element(section5_nodes, "BR", {});
			t119 = claim_space(section5_nodes);
			button15 = claim_element(section5_nodes, "BUTTON", { type: true, class: true });
			var button15_nodes = children(button15);
			t120 = claim_text(button15_nodes, "Button example");
			button15_nodes.forEach(detach_dev);
			section5_nodes.forEach(detach_dev);
			t121 = claim_space(nodes);
			br66 = claim_element(nodes, "BR", {});
			t122 = claim_space(nodes);
			br67 = claim_element(nodes, "BR", {});
			t123 = claim_space(nodes);
			h23 = claim_element(nodes, "H2", { class: true });
			var h23_nodes = children(h23);
			t124 = claim_text(h23_nodes, "Images");
			h23_nodes.forEach(detach_dev);
			t125 = claim_space(nodes);
			claim_component(picture1.$$.fragment, nodes);
			t126 = claim_space(nodes);
			br68 = claim_element(nodes, "BR", {});
			t127 = claim_space(nodes);
			br69 = claim_element(nodes, "BR", {});
			t128 = claim_space(nodes);
			claim_component(picture2.$$.fragment, nodes);
			t129 = claim_space(nodes);
			br70 = claim_element(nodes, "BR", {});
			t130 = claim_space(nodes);
			br71 = claim_element(nodes, "BR", {});
			t131 = claim_space(nodes);
			h24 = claim_element(nodes, "H2", { class: true });
			var h24_nodes = children(h24);
			t132 = claim_text(h24_nodes, "Other");
			h24_nodes.forEach(detach_dev);
			t133 = claim_space(nodes);
			p3 = claim_element(nodes, "P", { class: true });
			var p3_nodes = children(p3);
			strong = claim_element(p3_nodes, "STRONG", {});
			var strong_nodes = children(strong);
			t134 = claim_text(strong_nodes, "Try editing this file (src/routes/index.svelte) to test live reloading.");
			strong_nodes.forEach(detach_dev);
			p3_nodes.forEach(detach_dev);
			t135 = claim_space(nodes);
			p4 = claim_element(nodes, "P", { style: true, class: true });
			var p4_nodes = children(p4);
			t136 = claim_text(p4_nodes, "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Autem doloribus id laborum minima. Adipisci aperiam atque\n    autem commodi consequuntur debitis distinctio dolorem eum expedita ipsam iure labore laudantium maiores mollitia\n    nesciunt\n    nostrum optio perferendis porro, possimus quas quasi quisquam sint sunt totam unde. A atque blanditiis consequatur,\n    dolore\n    ducimus et id ipsam iusto labore, laudantium libero minus nam nostrum, numquam praesentium quisquam sint?\n    Consectetur dolorum\n    enim ex ipsum nam necessitatibus nisi nostrum odio omnis, quia recusandae temporibus totam. A aliquam, assumenda aut\n    autem\n    commodi consectetur dignissimos ducimus eius eos error ipsa ipsam molestias mollitia numquam praesentium quod rem\n    sapiente\n    sed sint totam! Ab aspernatur at culpa debitis dolores ea eius, excepturi ipsa laborum, minus numquam, optio\n    reiciendis\n    repudiandae similique temporibus! Blanditiis consequatur, dignissimos dolores eaque excepturi hic inventore ipsa\n    iste\n    nemo obcaecati provident quia, quibusdam ratione reiciendis saepe sequi vel veritatis vitae voluptate voluptatem.\n    Debitis\n    illum quaerat velit voluptatibus. A accusamus ad adipisci animi, aperiam architecto aspernatur aut beatae corporis\n    culpa\n    debitis dicta distinctio dolorem ducimus ea eligendi eos eveniet excepturi harum ipsum magnam maxime nesciunt\n    numquam obcaecati\n    perferendis, provident quaerat qui ratione reiciendis repellendus reprehenderit saepe sequi vel voluptas voluptate\n    voluptates\n    voluptatibus? Amet aperiam corporis deleniti deserunt, dolorem est exercitationem fuga hic impedit inventore itaque\n    laboriosam\n    laudantium, magnam necessitatibus nemo odio officiis perferendis perspiciatis quae qui quibusdam quis quod saepe sit\n    suscipit\n    tenetur ut vel veritatis voluptate voluptatibus. Aliquid cum cumque eum iure laboriosam natus rem. Accusantium alias\n    amet\n    corporis eaque enim explicabo fuga harum illo inventore itaque laudantium modi nemo nihil nulla possimus quae quas\n    quia quidem\n    quisquam recusandae repellendus sapiente sequi sit tempora, totam unde velit. Accusantium corporis cum dolor dolorum\n    iusto\n    laboriosam minus necessitatibus, nulla, omnis pariatur quae ullam voluptatem. Accusantium architecto, assumenda\n    commodi cumque\n    dignissimos doloribus eius et eum in iusto libero magnam, magni mollitia necessitatibus neque nesciunt perspiciatis\n    quidem\n    voluptas voluptatem voluptatibus? Aspernatur assumenda at consequatur deleniti eum expedita fuga in neque officia\n    pariatur,\n    provident quasi saepe sed sequi tempora tempore voluptates? Aliquam aperiam autem cum doloribus ducimus enim eum ex\n    expedita\n    explicabo id illo iste iure maiores modi nemo neque nisi nostrum officia officiis omnis perferendis perspiciatis\n    possimus\n    quae quas quibusdam quidem quis saepe sed sint sunt velit, vero vitae voluptatum? Amet debitis, dolore dolores eum\n    in ipsum\n    magnam mollitia nostrum quas quisquam quod rem sed sequi tenetur, voluptatibus. Ad adipisci aliquam animi aut autem\n    commodi\n    distinctio dolor eveniet impedit in iusto magni neque, odit optio pariatur placeat quaerat quo rem rerum sequi\n    suscipit\n    veniam vero voluptate voluptatem, voluptatum. Accusantium dicta dolores ea est libero rerum sapiente sit!\n    Accusantium cupiditate\n    eum ex molestiae praesentium provident saepe? Eveniet labore, rem. Aperiam, architecto at consectetur culpa cumque\n    debitis\n    dolorum eius eveniet fuga illo, in ipsum libero molestias necessitatibus nemo neque omnis perferendis perspiciatis\n    praesentium\n    quasi qui quia quod rerum saepe sed similique sunt suscipit tenetur totam ullam veritatis vitae voluptate\n    voluptatem. Assumenda\n    consequatur corporis dolorem doloremque ea inventore iusto sequi sit ut vitae! Adipisci alias aliquid aperiam\n    architecto asperiores\n    delectus dolore eaque eius eligendi fugiat incidunt ipsam iusto laborum magni maxime nam, necessitatibus nesciunt\n    omnis placeat\n    quaerat quibusdam rem, suscipit unde vel, veritatis vero voluptatum. Animi aut fuga impedit nostrum odit officia\n    quisquam,\n    repudiandae saepe suscipit voluptates! Ab autem beatae distinctio, dolorum eligendi est excepturi facere facilis\n    illo inventore\n    iure minus nemo nesciunt quaerat quibusdam repellendus sequi sint sit ullam voluptatem. Debitis ducimus excepturi\n    impedit\n    laudantium magnam nisi possimus, quae quod repellat sed sit tempora unde. Delectus eius laudantium quas quidem\n    similique sit\n    unde voluptas voluptate? Accusamus aut distinctio dolorum earum enim illo illum incidunt iure magni natus nulla\n    officiis\n    pariatur perspiciatis possimus quasi recusandae sint voluptas, voluptatibus. Ad aliquam amet, deleniti dignissimos,\n    eius enim\n    error excepturi hic magni quae qui quisquam repellendus rerum saepe similique suscipit temporibus vel voluptate. A\n    asperiores\n    at, dolores eligendi eum facere fuga impedit incidunt ipsam labore laborum magnam omnis placeat, quam quidem quos\n    similique\n    tenetur totam, vero voluptatibus. Ab asperiores autem cum delectus dicta dolor dolorum eos ex impedit libero nam\n    natus, necessitatibus\n    Lorem ipsum dolor sit amet, consectetur adipisicing elit. Autem doloribus id laborum minima. Adipisci aperiam atque\n    autem commodi consequuntur debitis distinctio dolorem eum expedita ipsam iure labore laudantium maiores mollitia\n    nesciunt\n    nostrum optio perferendis porro, possimus quas quasi quisquam sint sunt totam unde. A atque blanditiis consequatur,\n    dolore\n    ducimus et id ipsam iusto labore, laudantium libero minus nam nostrum, numquam praesentium quisquam sint?\n    Consectetur dolorum\n    enim ex ipsum nam necessitatibus nisi nostrum odio omnis, quia recusandae temporibus totam. A aliquam, assumenda aut\n    autem\n    commodi consectetur dignissimos ducimus eius eos error ipsa ipsam molestias mollitia numquam praesentium quod rem\n    sapiente\n    sed sint totam! Ab aspernatur at culpa debitis dolores ea eius, excepturi ipsa laborum, minus numquam, optio\n    reiciendis\n    repudiandae similique temporibus! Blanditiis consequatur, dignissimos dolores eaque excepturi hic inventore ipsa\n    iste\n    nemo obcaecati provident quia, quibusdam ratione reiciendis saepe sequi vel veritatis vitae voluptate voluptatem.\n    Debitis\n    illum quaerat velit voluptatibus. A accusamus ad adipisci animi, aperiam architecto aspernatur aut beatae corporis\n    culpa\n    debitis dicta distinctio dolorem ducimus ea eligendi eos eveniet excepturi harum ipsum magnam maxime nesciunt\n    numquam obcaecati\n    perferendis, provident quaerat qui ratione reiciendis repellendus reprehenderit saepe sequi vel voluptas voluptate\n    voluptates\n    voluptatibus? Amet aperiam corporis deleniti deserunt, dolorem est exercitationem fuga hic impedit inventore itaque\n    laboriosam\n    laudantium, magnam necessitatibus nemo odio officiis perferendis perspiciatis quae qui quibusdam quis quod saepe sit\n    suscipit\n    tenetur ut vel veritatis voluptate voluptatibus. Aliquid cum cumque eum iure laboriosam natus rem. Accusantium alias\n    amet\n    corporis eaque enim explicabo fuga harum illo inventore itaque laudantium modi nemo nihil nulla possimus quae quas\n    quia quidem\n    quisquam recusandae repellendus sapiente sequi sit tempora, totam unde velit. Accusantium corporis cum dolor dolorum\n    iusto\n    laboriosam minus necessitatibus, nulla, omnis pariatur quae ullam voluptatem. Accusantium architecto, assumenda\n    commodi cumque\n    dignissimos doloribus eius et eum in iusto libero magnam, magni mollitia necessitatibus neque nesciunt perspiciatis\n    quidem\n    voluptas voluptatem voluptatibus? Aspernatur assumenda at consequatur deleniti eum expedita fuga in neque officia\n    pariatur,\n    provident quasi saepe sed sequi tempora tempore voluptates? Aliquam aperiam autem cum doloribus ducimus enim eum ex\n    expedita\n    explicabo id illo iste iure maiores modi nemo neque nisi nostrum officia officiis omnis perferendis perspiciatis\n    possimus\n    quae quas quibusdam quidem quis saepe sed sint sunt velit, vero vitae voluptatum? Amet debitis, dolore dolores eum\n    in ipsum\n    magnam mollitia nostrum quas quisquam quod rem sed sequi tenetur, voluptatibus. Ad adipisci aliquam animi aut autem\n    commodi\n    distinctio dolor eveniet impedit in iusto magni neque, odit optio pariatur placeat quaerat quo rem rerum sequi\n    suscipit\n    veniam vero voluptate voluptatem, voluptatum. Accusantium dicta dolores ea est libero rerum sapiente sit!\n    Accusantium cupiditate\n    eum ex molestiae praesentium provident saepe? Eveniet labore, rem. Aperiam, architecto at consectetur culpa cumque\n    debitis\n    dolorum eius eveniet fuga illo, in ipsum libero molestias necessitatibus nemo neque omnis perferendis perspiciatis\n    praesentium\n    quasi qui quia quod rerum saepe sed similique sunt suscipit tenetur totam ullam veritatis vitae voluptate\n    voluptatem. Assumenda\n    consequatur corporis dolorem doloremque ea inventore iusto sequi sit ut vitae! Adipisci alias aliquid aperiam\n    architecto asperiores\n    delectus dolore eaque eius eligendi fugiat incidunt ipsam iusto laborum magni maxime nam, necessitatibus nesciunt\n    omnis placeat\n    quaerat quibusdam rem, suscipit unde vel, veritatis vero voluptatum. Animi aut fuga impedit nostrum odit officia\n    quisquam,\n    repudiandae saepe suscipit voluptates! Ab autem beatae distinctio, dolorum eligendi est excepturi facere facilis\n    illo inventore\n    iure minus nemo nesciunt quaerat quibusdam repellendus sequi sint sit ullam voluptatem. Debitis ducimus excepturi\n    impedit\n    laudantium magnam nisi possimus, quae quod repellat sed sit tempora unde. Delectus eius laudantium quas quidem\n    similique sit\n    unde voluptas voluptate? Accusamus aut distinctio dolorum earum enim illo illum incidunt iure magni natus nulla\n    officiis\n    pariatur perspiciatis possimus quasi recusandae sint voluptas, voluptatibus. Ad aliquam amet, deleniti dignissimos,\n    eius enim\n    error excepturi hic magni quae qui quisquam repellendus rerum saepe similique suscipit temporibus vel voluptate. A\n    asperiores\n    at, dolores eligendi eum facere fuga impedit incidunt ipsam labore laborum magnam omnis placeat, quam quidem quos\n    similique\n    tenetur totam, vero voluptatibus. Ab asperiores autem cum delectus dicta dolor dolorum eos ex impedit libero nam\n    natus, necessitatibus\n    Lorem ipsum dolor sit amet, consectetur adipisicing elit. Autem doloribus id laborum minima. Adipisci aperiam atque\n    autem commodi consequuntur debitis distinctio dolorem eum expedita ipsam iure labore laudantium maiores mollitia\n    nesciunt\n    nostrum optio perferendis porro, possimus quas quasi quisquam sint sunt totam unde. A atque blanditiis consequatur,\n    dolore\n    ducimus et id ipsam iusto labore, laudantium libero minus nam nostrum, numquam praesentium quisquam sint?\n    Consectetur dolorum\n    enim ex ipsum nam necessitatibus nisi nostrum odio omnis, quia recusandae temporibus totam. A aliquam, assumenda aut\n    autem\n    commodi consectetur dignissimos ducimus eius eos error ipsa ipsam molestias mollitia numquam praesentium quod rem\n    sapiente\n    sed sint totam! Ab aspernatur at culpa debitis dolores ea eius, excepturi ipsa laborum, minus numquam, optio\n    reiciendis\n    repudiandae similique temporibus! Blanditiis consequatur, dignissimos dolores eaque excepturi hic inventore ipsa\n    iste\n    nemo obcaecati provident quia, quibusdam ratione reiciendis saepe sequi vel veritatis vitae voluptate voluptatem.\n    Debitis\n    illum quaerat velit voluptatibus. A accusamus ad adipisci animi, aperiam architecto aspernatur aut beatae corporis\n    culpa\n    debitis dicta distinctio dolorem ducimus ea eligendi eos eveniet excepturi harum ipsum magnam maxime nesciunt\n    numquam obcaecati\n    perferendis, provident quaerat qui ratione reiciendis repellendus reprehenderit saepe sequi vel voluptas voluptate\n    voluptates\n    voluptatibus? Amet aperiam corporis deleniti deserunt, dolorem est exercitationem fuga hic impedit inventore itaque\n    laboriosam\n    laudantium, magnam necessitatibus nemo odio officiis perferendis perspiciatis quae qui quibusdam quis quod saepe sit\n    suscipit\n    tenetur ut vel veritatis voluptate voluptatibus. Aliquid cum cumque eum iure laboriosam natus rem. Accusantium alias\n    amet\n    corporis eaque enim explicabo fuga harum illo inventore itaque laudantium modi nemo nihil nulla possimus quae quas\n    quia quidem\n    quisquam recusandae repellendus sapiente sequi sit tempora, totam unde velit. Accusantium corporis cum dolor dolorum\n    iusto\n    laboriosam minus necessitatibus, nulla, omnis pariatur quae ullam voluptatem. Accusantium architecto, assumenda\n    commodi cumque\n    dignissimos doloribus eius et eum in iusto libero magnam, magni mollitia necessitatibus neque nesciunt perspiciatis\n    quidem\n    voluptas voluptatem voluptatibus? Aspernatur assumenda at consequatur deleniti eum expedita fuga in neque officia\n    pariatur,\n    provident quasi saepe sed sequi tempora tempore voluptates? Aliquam aperiam autem cum doloribus ducimus enim eum ex\n    expedita\n    explicabo id illo iste iure maiores modi nemo neque nisi nostrum officia officiis omnis perferendis perspiciatis\n    possimus\n    quae quas quibusdam quidem quis saepe sed sint sunt velit, vero vitae voluptatum? Amet debitis, dolore dolores eum\n    in ipsum\n    magnam mollitia nostrum quas quisquam quod rem sed sequi tenetur, voluptatibus. Ad adipisci aliquam animi aut autem\n    commodi\n    distinctio dolor eveniet impedit in iusto magni neque, odit optio pariatur placeat quaerat quo rem rerum sequi\n    suscipit\n    veniam vero voluptate voluptatem, voluptatum. Accusantium dicta dolores ea est libero rerum sapiente sit!\n    Accusantium cupiditate\n    eum ex molestiae praesentium provident saepe? Eveniet labore, rem. Aperiam, architecto at consectetur culpa cumque\n    debitis\n    dolorum eius eveniet fuga illo, in ipsum libero molestias necessitatibus nemo neque omnis perferendis perspiciatis\n    praesentium\n    quasi qui quia quod rerum saepe sed similique sunt suscipit tenetur totam ullam veritatis vitae voluptate\n    voluptatem. Assumenda\n    consequatur corporis dolorem doloremque ea inventore iusto sequi sit ut vitae! Adipisci alias aliquid aperiam\n    architecto asperiores\n    delectus dolore eaque eius eligendi fugiat incidunt ipsam iusto laborum magni maxime nam, necessitatibus nesciunt\n    omnis placeat\n    quaerat quibusdam rem, suscipit unde vel, veritatis vero voluptatum. Animi aut fuga impedit nostrum odit officia\n    quisquam,\n    repudiandae saepe suscipit voluptates! Ab autem beatae distinctio, dolorum eligendi est excepturi facere facilis\n    illo inventore\n    iure minus nemo nesciunt quaerat quibusdam repellendus sequi sint sit ullam voluptatem. Debitis ducimus excepturi\n    impedit\n    laudantium magnam nisi possimus, quae quod repellat sed sit tempora unde. Delectus eius laudantium quas quidem\n    similique sit\n    unde voluptas voluptate? Accusamus aut distinctio dolorum earum enim illo illum incidunt iure magni natus nulla\n    officiis\n    pariatur perspiciatis possimus quasi recusandae sint voluptas, voluptatibus. Ad aliquam amet, deleniti dignissimos,\n    eius enim\n    error excepturi hic magni quae qui quisquam repellendus rerum saepe similique suscipit temporibus vel voluptate. A\n    asperiores\n    at, dolores eligendi eum facere fuga impedit incidunt ipsam labore laborum magnam omnis placeat, quam quidem quos\n    similique\n    tenetur totam, vero voluptatibus. Ab asperiores autem cum delectus dicta dolor dolorum eos ex impedit libero nam\n    natus, necessitatibus");
			p4_nodes.forEach(detach_dev);
			this.h();
		},
		h: function hydrate() {
			document.title = "Test Title";
			attr_dev(div0, "class", "top-pic svelte-1kmv00v");
			add_location(div0, file, 53, 8, 792);
			attr_dev(li0, "class", "svelte-1kmv00v");
			add_location(li0, file, 58, 12, 951);
			attr_dev(li1, "class", "svelte-1kmv00v");
			add_location(li1, file, 61, 12, 1071);
			attr_dev(li2, "class", "svelte-1kmv00v");
			add_location(li2, file, 64, 12, 1192);
			option0.__value = "20";
			option0.value = option0.__value;
			add_location(option0, file, 78, 20, 1665);
			option1.__value = "500";
			option1.value = option1.__value;
			add_location(option1, file, 79, 20, 1705);
			option2.__value = "1000";
			option2.value = option2.__value;
			add_location(option2, file, 80, 20, 1746);
			attr_dev(datalist0, "id", "sum-suggestions");
			add_location(datalist0, file, 77, 16, 1613);
			attr_dev(li3, "class", "svelte-1kmv00v");
			add_location(li3, file, 67, 12, 1314);
			attr_dev(ul, "class", "options svelte-1kmv00v");
			add_location(ul, file, 57, 8, 918);
			attr_dev(section0, "class", "top svelte-1kmv00v");
			add_location(section0, file, 52, 4, 762);
			set_style(section1, "padding", "20px 5px 20px 0");
			add_location(section1, file, 86, 4, 1848);
			attr_dev(section2, "class", "top-wrap svelte-1kmv00v");
			add_location(section2, file, 48, 0, 697);
			add_location(br0, file, 94, 4, 2020);
			add_location(br1, file, 95, 4, 2029);
			add_location(br2, file, 99, 4, 2090);
			add_location(br3, file, 100, 4, 2099);
			add_location(br4, file, 104, 4, 2158);
			add_location(br5, file, 105, 4, 2167);
			add_location(br6, file, 109, 4, 2227);
			add_location(br7, file, 110, 4, 2236);
			add_location(br8, file, 114, 4, 2294);
			add_location(br9, file, 115, 4, 2303);
			add_location(br10, file, 119, 4, 2364);
			add_location(br11, file, 120, 4, 2373);
			add_location(br12, file, 124, 4, 2432);
			add_location(br13, file, 125, 4, 2441);
			add_location(br14, file, 129, 4, 2504);
			add_location(br15, file, 130, 4, 2513);
			add_location(section3, file, 92, 0, 2005);
			attr_dev(span, "class", "text-success");
			add_location(span, file, 135, 10, 2588);
			attr_dev(h1, "class", "svelte-1kmv00v");
			add_location(h1, file, 135, 0, 2578);
			add_location(br16, file, 137, 0, 2637);
			add_location(br17, file, 138, 0, 2642);
			attr_dev(h20, "class", "text-success");
			add_location(h20, file, 140, 0, 2648);
			add_location(br18, file, 142, 0, 2690);
			add_location(br19, file, 143, 0, 2695);
			attr_dev(p0, "class", "svelte-1kmv00v");
			add_location(p0, file, 145, 0, 2701);
			attr_dev(p1, "class", "svelte-1kmv00v");
			add_location(p1, file, 146, 0, 2737);
			attr_dev(p2, "class", "svelte-1kmv00v");
			add_location(p2, file, 147, 0, 2781);
			add_location(br20, file, 149, 0, 2827);
			add_location(br21, file, 150, 0, 2832);
			attr_dev(h21, "class", "text-warning");
			add_location(h21, file, 152, 0, 2838);
			add_location(br22, file, 154, 0, 2890);
			add_location(br23, file, 155, 0, 2895);
			add_location(br24, file, 159, 8, 3002);
			add_location(br25, file, 161, 8, 3031);
			add_location(br26, file, 162, 8, 3044);
			add_location(br27, file, 164, 8, 3120);
			add_location(br28, file, 165, 8, 3133);
			add_location(br29, file, 167, 8, 3222);
			add_location(br30, file, 168, 8, 3235);
			add_location(br31, file, 170, 8, 3341);
			add_location(br32, file, 171, 8, 3354);
			add_location(br33, file, 175, 8, 3481);
			add_location(br34, file, 176, 8, 3494);
			add_location(br35, file, 178, 8, 3601);
			add_location(br36, file, 179, 8, 3614);
			add_location(br37, file, 181, 8, 3720);
			add_location(br38, file, 182, 8, 3733);
			add_location(div1, file, 158, 4, 2988);
			add_location(br39, file, 186, 8, 3768);
			add_location(br40, file, 188, 8, 3806);
			add_location(br41, file, 189, 8, 3819);
			add_location(br42, file, 191, 8, 3904);
			add_location(br43, file, 192, 8, 3917);
			add_location(br44, file, 194, 8, 4015);
			add_location(br45, file, 195, 8, 4028);
			add_location(br46, file, 197, 8, 4126);
			add_location(br47, file, 198, 8, 4139);
			add_location(br48, file, 202, 8, 4275);
			add_location(br49, file, 203, 8, 4288);
			add_location(br50, file, 205, 8, 4404);
			add_location(br51, file, 206, 8, 4417);
			add_location(br52, file, 208, 8, 4532);
			add_location(br53, file, 209, 8, 4545);
			add_location(div2, file, 185, 4, 3754);
			set_style(section4, "text-align", "center");
			set_style(section4, "display", "flex");
			set_style(section4, "justify-content", "space-around");
			add_location(section4, file, 157, 0, 2901);
			add_location(br54, file, 213, 0, 4573);
			add_location(br55, file, 214, 0, 4578);
			attr_dev(h22, "class", "text-danger");
			add_location(h22, file, 216, 0, 4584);
			add_location(br56, file, 218, 0, 4627);
			add_location(br57, file, 219, 0, 4632);
			add_location(br58, file, 224, 4, 4697);
			add_location(br59, file, 225, 4, 4706);
			add_location(br60, file, 229, 4, 4789);
			add_location(br61, file, 230, 4, 4798);
			option3.__value = "1";
			option3.value = option3.__value;
			add_location(option3, file, 235, 8, 4887);
			option4.__value = "2";
			option4.value = option4.__value;
			add_location(option4, file, 236, 8, 4914);
			option5.__value = "3";
			option5.value = option5.__value;
			add_location(option5, file, 237, 8, 4941);
			attr_dev(datalist1, "id", "lis");
			add_location(datalist1, file, 234, 4, 4859);
			attr_dev(label, "for", "input");
			attr_dev(label, "class", "text-success");
			add_location(label, file, 240, 4, 4981);
			attr_dev(input12, "id", "input");
			attr_dev(input12, "type", "text");
			input12.value = "text";
			add_location(input12, file, 241, 4, 5039);
			add_location(br62, file, 243, 4, 5088);
			add_location(br63, file, 244, 4, 5097);
			attr_dev(a, "href", ".");
			attr_dev(a, "class", "text-warning");
			add_location(a, file, 246, 4, 5107);
			add_location(br64, file, 248, 4, 5159);
			add_location(br65, file, 249, 4, 5168);
			attr_dev(button15, "type", "button");
			attr_dev(button15, "class", "text-info");
			add_location(button15, file, 251, 4, 5178);
			add_location(section5, file, 221, 0, 4638);
			add_location(br66, file, 254, 0, 5254);
			add_location(br67, file, 255, 0, 5259);
			attr_dev(h23, "class", "text-info");
			add_location(h23, file, 257, 0, 5265);
			add_location(br68, file, 263, 0, 5389);
			add_location(br69, file, 264, 0, 5394);
			add_location(br70, file, 268, 0, 5466);
			add_location(br71, file, 269, 0, 5471);
			attr_dev(h24, "class", "text-success");
			add_location(h24, file, 271, 0, 5477);
			add_location(strong, file, 274, 4, 5522);
			attr_dev(p3, "class", "svelte-1kmv00v");
			add_location(p3, file, 273, 0, 5514);
			set_style(p4, "text-align", "justify");
			attr_dev(p4, "class", "svelte-1kmv00v");
			add_location(p4, file, 279, 0, 5631);
		},
		m: function mount(target, anchor) {
			insert_dev(target, t0, anchor);
			insert_dev(target, section2, anchor);
			mount_component(icon, section2, null);
			append_dev(section2, t1);
			append_dev(section2, section0);
			append_dev(section0, div0);
			mount_component(picture0, div0, null);
			append_dev(section0, t2);
			append_dev(section0, ul);
			append_dev(ul, li0);
			mount_component(button0, li0, null);
			append_dev(ul, t3);
			append_dev(ul, li1);
			mount_component(button1, li1, null);
			append_dev(ul, t4);
			append_dev(ul, li2);
			mount_component(button2, li2, null);
			append_dev(ul, t5);
			append_dev(ul, li3);
			mount_component(input0, li3, null);
			append_dev(li3, t6);
			append_dev(li3, datalist0);
			append_dev(datalist0, option0);
			append_dev(datalist0, option1);
			append_dev(datalist0, option2);
			append_dev(section2, t7);
			append_dev(section2, section1);
			mount_component(progress, section1, null);
			insert_dev(target, t8, anchor);
			insert_dev(target, section3, anchor);
			append_dev(section3, br0);
			append_dev(section3, t9);
			append_dev(section3, br1);
			append_dev(section3, t10);
			mount_component(input1, section3, null);
			append_dev(section3, t11);
			append_dev(section3, br2);
			append_dev(section3, t12);
			append_dev(section3, br3);
			append_dev(section3, t13);
			mount_component(input2, section3, null);
			append_dev(section3, t14);
			append_dev(section3, br4);
			append_dev(section3, t15);
			append_dev(section3, br5);
			append_dev(section3, t16);
			mount_component(input3, section3, null);
			append_dev(section3, t17);
			append_dev(section3, br6);
			append_dev(section3, t18);
			append_dev(section3, br7);
			append_dev(section3, t19);
			mount_component(input4, section3, null);
			append_dev(section3, t20);
			append_dev(section3, br8);
			append_dev(section3, t21);
			append_dev(section3, br9);
			append_dev(section3, t22);
			mount_component(input5, section3, null);
			append_dev(section3, t23);
			append_dev(section3, br10);
			append_dev(section3, t24);
			append_dev(section3, br11);
			append_dev(section3, t25);
			mount_component(input6, section3, null);
			append_dev(section3, t26);
			append_dev(section3, br12);
			append_dev(section3, t27);
			append_dev(section3, br13);
			append_dev(section3, t28);
			mount_component(input7, section3, null);
			append_dev(section3, t29);
			append_dev(section3, br14);
			append_dev(section3, t30);
			append_dev(section3, br15);
			append_dev(section3, t31);
			mount_component(input8, section3, null);
			insert_dev(target, t32, anchor);
			insert_dev(target, h1, anchor);
			append_dev(h1, t33);
			append_dev(h1, span);
			append_dev(span, t34);
			append_dev(h1, t35);
			insert_dev(target, t36, anchor);
			insert_dev(target, br16, anchor);
			insert_dev(target, t37, anchor);
			insert_dev(target, br17, anchor);
			insert_dev(target, t38, anchor);
			insert_dev(target, h20, anchor);
			append_dev(h20, t39);
			insert_dev(target, t40, anchor);
			insert_dev(target, br18, anchor);
			insert_dev(target, t41, anchor);
			insert_dev(target, br19, anchor);
			insert_dev(target, t42, anchor);
			insert_dev(target, p0, anchor);
			append_dev(p0, t43);
			insert_dev(target, t44, anchor);
			insert_dev(target, p1, anchor);
			append_dev(p1, t45);
			insert_dev(target, t46, anchor);
			insert_dev(target, p2, anchor);
			append_dev(p2, t47);
			insert_dev(target, t48, anchor);
			insert_dev(target, br20, anchor);
			insert_dev(target, t49, anchor);
			insert_dev(target, br21, anchor);
			insert_dev(target, t50, anchor);
			insert_dev(target, h21, anchor);
			append_dev(h21, t51);
			insert_dev(target, t52, anchor);
			insert_dev(target, br22, anchor);
			insert_dev(target, t53, anchor);
			insert_dev(target, br23, anchor);
			insert_dev(target, t54, anchor);
			insert_dev(target, section4, anchor);
			append_dev(section4, div1);
			append_dev(div1, br24);
			append_dev(div1, t55);
			append_dev(div1, br25);
			append_dev(div1, t56);
			append_dev(div1, br26);
			append_dev(div1, t57);
			mount_component(button3, div1, null);
			append_dev(div1, t58);
			append_dev(div1, br27);
			append_dev(div1, t59);
			append_dev(div1, br28);
			append_dev(div1, t60);
			mount_component(button4, div1, null);
			append_dev(div1, t61);
			append_dev(div1, br29);
			append_dev(div1, t62);
			append_dev(div1, br30);
			append_dev(div1, t63);
			mount_component(button5, div1, null);
			append_dev(div1, t64);
			append_dev(div1, br31);
			append_dev(div1, t65);
			append_dev(div1, br32);
			append_dev(div1, t66);
			mount_component(button6, div1, null);
			append_dev(div1, t67);
			append_dev(div1, br33);
			append_dev(div1, t68);
			append_dev(div1, br34);
			append_dev(div1, t69);
			mount_component(button7, div1, null);
			append_dev(div1, t70);
			append_dev(div1, br35);
			append_dev(div1, t71);
			append_dev(div1, br36);
			append_dev(div1, t72);
			mount_component(button8, div1, null);
			append_dev(div1, t73);
			append_dev(div1, br37);
			append_dev(div1, t74);
			append_dev(div1, br38);
			append_dev(section4, t75);
			append_dev(section4, div2);
			append_dev(div2, br39);
			append_dev(div2, t76);
			append_dev(div2, br40);
			append_dev(div2, t77);
			append_dev(div2, br41);
			append_dev(div2, t78);
			mount_component(button9, div2, null);
			append_dev(div2, t79);
			append_dev(div2, br42);
			append_dev(div2, t80);
			append_dev(div2, br43);
			append_dev(div2, t81);
			mount_component(button10, div2, null);
			append_dev(div2, t82);
			append_dev(div2, br44);
			append_dev(div2, t83);
			append_dev(div2, br45);
			append_dev(div2, t84);
			mount_component(button11, div2, null);
			append_dev(div2, t85);
			append_dev(div2, br46);
			append_dev(div2, t86);
			append_dev(div2, br47);
			append_dev(div2, t87);
			mount_component(button12, div2, null);
			append_dev(div2, t88);
			append_dev(div2, br48);
			append_dev(div2, t89);
			append_dev(div2, br49);
			append_dev(div2, t90);
			mount_component(button13, div2, null);
			append_dev(div2, t91);
			append_dev(div2, br50);
			append_dev(div2, t92);
			append_dev(div2, br51);
			append_dev(div2, t93);
			mount_component(button14, div2, null);
			append_dev(div2, t94);
			append_dev(div2, br52);
			append_dev(div2, t95);
			append_dev(div2, br53);
			insert_dev(target, t96, anchor);
			insert_dev(target, br54, anchor);
			insert_dev(target, t97, anchor);
			insert_dev(target, br55, anchor);
			insert_dev(target, t98, anchor);
			insert_dev(target, h22, anchor);
			append_dev(h22, t99);
			insert_dev(target, t100, anchor);
			insert_dev(target, br56, anchor);
			insert_dev(target, t101, anchor);
			insert_dev(target, br57, anchor);
			insert_dev(target, t102, anchor);
			insert_dev(target, section5, anchor);
			mount_component(input9, section5, null);
			append_dev(section5, t103);
			append_dev(section5, br58);
			append_dev(section5, t104);
			append_dev(section5, br59);
			append_dev(section5, t105);
			mount_component(input10, section5, null);
			append_dev(section5, t106);
			append_dev(section5, br60);
			append_dev(section5, t107);
			append_dev(section5, br61);
			append_dev(section5, t108);
			mount_component(input11, section5, null);
			append_dev(section5, t109);
			append_dev(section5, datalist1);
			append_dev(datalist1, option3);
			append_dev(datalist1, option4);
			append_dev(datalist1, option5);
			append_dev(section5, t110);
			append_dev(section5, label);
			append_dev(label, t111);
			append_dev(section5, t112);
			append_dev(section5, input12);
			append_dev(section5, t113);
			append_dev(section5, br62);
			append_dev(section5, t114);
			append_dev(section5, br63);
			append_dev(section5, t115);
			append_dev(section5, a);
			append_dev(a, t116);
			append_dev(section5, t117);
			append_dev(section5, br64);
			append_dev(section5, t118);
			append_dev(section5, br65);
			append_dev(section5, t119);
			append_dev(section5, button15);
			append_dev(button15, t120);
			insert_dev(target, t121, anchor);
			insert_dev(target, br66, anchor);
			insert_dev(target, t122, anchor);
			insert_dev(target, br67, anchor);
			insert_dev(target, t123, anchor);
			insert_dev(target, h23, anchor);
			append_dev(h23, t124);
			insert_dev(target, t125, anchor);
			mount_component(picture1, target, anchor);
			insert_dev(target, t126, anchor);
			insert_dev(target, br68, anchor);
			insert_dev(target, t127, anchor);
			insert_dev(target, br69, anchor);
			insert_dev(target, t128, anchor);
			mount_component(picture2, target, anchor);
			insert_dev(target, t129, anchor);
			insert_dev(target, br70, anchor);
			insert_dev(target, t130, anchor);
			insert_dev(target, br71, anchor);
			insert_dev(target, t131, anchor);
			insert_dev(target, h24, anchor);
			append_dev(h24, t132);
			insert_dev(target, t133, anchor);
			insert_dev(target, p3, anchor);
			append_dev(p3, strong);
			append_dev(strong, t134);
			insert_dev(target, t135, anchor);
			insert_dev(target, p4, anchor);
			append_dev(p4, t136);
			current = true;
		},
		p: function update(ctx, [dirty]) {
			const button0_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button0_changes.$$scope = { dirty, ctx };
			}

			button0.$set(button0_changes);
			const button1_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button1_changes.$$scope = { dirty, ctx };
			}

			button1.$set(button1_changes);
			const button2_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button2_changes.$$scope = { dirty, ctx };
			}

			button2.$set(button2_changes);
			const button3_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button3_changes.$$scope = { dirty, ctx };
			}

			button3.$set(button3_changes);
			const button4_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button4_changes.$$scope = { dirty, ctx };
			}

			button4.$set(button4_changes);
			const button5_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button5_changes.$$scope = { dirty, ctx };
			}

			button5.$set(button5_changes);
			const button6_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button6_changes.$$scope = { dirty, ctx };
			}

			button6.$set(button6_changes);
			const button7_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button7_changes.$$scope = { dirty, ctx };
			}

			button7.$set(button7_changes);
			const button8_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button8_changes.$$scope = { dirty, ctx };
			}

			button8.$set(button8_changes);
			const button9_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button9_changes.$$scope = { dirty, ctx };
			}

			button9.$set(button9_changes);
			const button10_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button10_changes.$$scope = { dirty, ctx };
			}

			button10.$set(button10_changes);
			const button11_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button11_changes.$$scope = { dirty, ctx };
			}

			button11.$set(button11_changes);
			const button12_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button12_changes.$$scope = { dirty, ctx };
			}

			button12.$set(button12_changes);
			const button13_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button13_changes.$$scope = { dirty, ctx };
			}

			button13.$set(button13_changes);
			const button14_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				button14_changes.$$scope = { dirty, ctx };
			}

			button14.$set(button14_changes);
			const picture1_changes = {};

			if (dirty & /*$$scope*/ 32768) {
				picture1_changes.$$scope = { dirty, ctx };
			}

			picture1.$set(picture1_changes);
		},
		i: function intro(local) {
			if (current) return;
			transition_in(icon.$$.fragment, local);
			transition_in(picture0.$$.fragment, local);
			transition_in(button0.$$.fragment, local);
			transition_in(button1.$$.fragment, local);
			transition_in(button2.$$.fragment, local);
			transition_in(input0.$$.fragment, local);
			transition_in(progress.$$.fragment, local);
			transition_in(input1.$$.fragment, local);
			transition_in(input2.$$.fragment, local);
			transition_in(input3.$$.fragment, local);
			transition_in(input4.$$.fragment, local);
			transition_in(input5.$$.fragment, local);
			transition_in(input6.$$.fragment, local);
			transition_in(input7.$$.fragment, local);
			transition_in(input8.$$.fragment, local);
			transition_in(button3.$$.fragment, local);
			transition_in(button4.$$.fragment, local);
			transition_in(button5.$$.fragment, local);
			transition_in(button6.$$.fragment, local);
			transition_in(button7.$$.fragment, local);
			transition_in(button8.$$.fragment, local);
			transition_in(button9.$$.fragment, local);
			transition_in(button10.$$.fragment, local);
			transition_in(button11.$$.fragment, local);
			transition_in(button12.$$.fragment, local);
			transition_in(button13.$$.fragment, local);
			transition_in(button14.$$.fragment, local);
			transition_in(input9.$$.fragment, local);
			transition_in(input10.$$.fragment, local);
			transition_in(input11.$$.fragment, local);
			transition_in(picture1.$$.fragment, local);
			transition_in(picture2.$$.fragment, local);
			current = true;
		},
		o: function outro(local) {
			transition_out(icon.$$.fragment, local);
			transition_out(picture0.$$.fragment, local);
			transition_out(button0.$$.fragment, local);
			transition_out(button1.$$.fragment, local);
			transition_out(button2.$$.fragment, local);
			transition_out(input0.$$.fragment, local);
			transition_out(progress.$$.fragment, local);
			transition_out(input1.$$.fragment, local);
			transition_out(input2.$$.fragment, local);
			transition_out(input3.$$.fragment, local);
			transition_out(input4.$$.fragment, local);
			transition_out(input5.$$.fragment, local);
			transition_out(input6.$$.fragment, local);
			transition_out(input7.$$.fragment, local);
			transition_out(input8.$$.fragment, local);
			transition_out(button3.$$.fragment, local);
			transition_out(button4.$$.fragment, local);
			transition_out(button5.$$.fragment, local);
			transition_out(button6.$$.fragment, local);
			transition_out(button7.$$.fragment, local);
			transition_out(button8.$$.fragment, local);
			transition_out(button9.$$.fragment, local);
			transition_out(button10.$$.fragment, local);
			transition_out(button11.$$.fragment, local);
			transition_out(button12.$$.fragment, local);
			transition_out(button13.$$.fragment, local);
			transition_out(button14.$$.fragment, local);
			transition_out(input9.$$.fragment, local);
			transition_out(input10.$$.fragment, local);
			transition_out(input11.$$.fragment, local);
			transition_out(picture1.$$.fragment, local);
			transition_out(picture2.$$.fragment, local);
			current = false;
		},
		d: function destroy(detaching) {
			if (detaching) detach_dev(t0);
			if (detaching) detach_dev(section2);
			destroy_component(icon);
			destroy_component(picture0);
			destroy_component(button0);
			destroy_component(button1);
			destroy_component(button2);
			destroy_component(input0);
			destroy_component(progress);
			if (detaching) detach_dev(t8);
			if (detaching) detach_dev(section3);
			destroy_component(input1);
			destroy_component(input2);
			destroy_component(input3);
			destroy_component(input4);
			destroy_component(input5);
			destroy_component(input6);
			destroy_component(input7);
			destroy_component(input8);
			if (detaching) detach_dev(t32);
			if (detaching) detach_dev(h1);
			if (detaching) detach_dev(t36);
			if (detaching) detach_dev(br16);
			if (detaching) detach_dev(t37);
			if (detaching) detach_dev(br17);
			if (detaching) detach_dev(t38);
			if (detaching) detach_dev(h20);
			if (detaching) detach_dev(t40);
			if (detaching) detach_dev(br18);
			if (detaching) detach_dev(t41);
			if (detaching) detach_dev(br19);
			if (detaching) detach_dev(t42);
			if (detaching) detach_dev(p0);
			if (detaching) detach_dev(t44);
			if (detaching) detach_dev(p1);
			if (detaching) detach_dev(t46);
			if (detaching) detach_dev(p2);
			if (detaching) detach_dev(t48);
			if (detaching) detach_dev(br20);
			if (detaching) detach_dev(t49);
			if (detaching) detach_dev(br21);
			if (detaching) detach_dev(t50);
			if (detaching) detach_dev(h21);
			if (detaching) detach_dev(t52);
			if (detaching) detach_dev(br22);
			if (detaching) detach_dev(t53);
			if (detaching) detach_dev(br23);
			if (detaching) detach_dev(t54);
			if (detaching) detach_dev(section4);
			destroy_component(button3);
			destroy_component(button4);
			destroy_component(button5);
			destroy_component(button6);
			destroy_component(button7);
			destroy_component(button8);
			destroy_component(button9);
			destroy_component(button10);
			destroy_component(button11);
			destroy_component(button12);
			destroy_component(button13);
			destroy_component(button14);
			if (detaching) detach_dev(t96);
			if (detaching) detach_dev(br54);
			if (detaching) detach_dev(t97);
			if (detaching) detach_dev(br55);
			if (detaching) detach_dev(t98);
			if (detaching) detach_dev(h22);
			if (detaching) detach_dev(t100);
			if (detaching) detach_dev(br56);
			if (detaching) detach_dev(t101);
			if (detaching) detach_dev(br57);
			if (detaching) detach_dev(t102);
			if (detaching) detach_dev(section5);
			destroy_component(input9);
			destroy_component(input10);
			destroy_component(input11);
			if (detaching) detach_dev(t121);
			if (detaching) detach_dev(br66);
			if (detaching) detach_dev(t122);
			if (detaching) detach_dev(br67);
			if (detaching) detach_dev(t123);
			if (detaching) detach_dev(h23);
			if (detaching) detach_dev(t125);
			destroy_component(picture1, detaching);
			if (detaching) detach_dev(t126);
			if (detaching) detach_dev(br68);
			if (detaching) detach_dev(t127);
			if (detaching) detach_dev(br69);
			if (detaching) detach_dev(t128);
			destroy_component(picture2, detaching);
			if (detaching) detach_dev(t129);
			if (detaching) detach_dev(br70);
			if (detaching) detach_dev(t130);
			if (detaching) detach_dev(br71);
			if (detaching) detach_dev(t131);
			if (detaching) detach_dev(h24);
			if (detaching) detach_dev(t133);
			if (detaching) detach_dev(p3);
			if (detaching) detach_dev(t135);
			if (detaching) detach_dev(p4);
		}
	};

	dispatch_dev("SvelteRegisterBlock", {
		block,
		id: create_fragment.name,
		type: "component",
		source: "",
		ctx
	});

	return block;
}

function instance($$self) {
	const click_handler = e => console.log(e);
	const click_handler_1 = e => console.log(e);
	const click_handler_2 = e => console.log(e);
	const click_handler_3 = e => console.log(e);
	const click_handler_4 = e => console.log(e);
	const click_handler_5 = e => console.log(e);
	const click_handler_6 = e => console.log(e);
	const click_handler_7 = e => console.log(e);
	const click_handler_8 = e => console.log(e);
	const click_handler_9 = e => console.log(e);
	const click_handler_10 = e => console.log(e);
	const click_handler_11 = e => console.log(e);
	const click_handler_12 = e => console.log(e);
	const click_handler_13 = e => console.log(e);
	const click_handler_14 = e => console.log(e);

	$$self.$capture_state = () => {
		return {};
	};

	$$self.$inject_state = $$props => {
		
	};

	return [
		click_handler,
		click_handler_1,
		click_handler_2,
		click_handler_3,
		click_handler_4,
		click_handler_5,
		click_handler_6,
		click_handler_7,
		click_handler_8,
		click_handler_9,
		click_handler_10,
		click_handler_11,
		click_handler_12,
		click_handler_13,
		click_handler_14
	];
}

class Routes extends SvelteComponentDev {
	constructor(options) {
		super(options);
		init(this, options, instance, create_fragment, safe_not_equal, {});

		dispatch_dev("SvelteRegisterComponent", {
			component: this,
			tagName: "Routes",
			options,
			id: create_fragment.name
		});
	}
}

export default Routes;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
